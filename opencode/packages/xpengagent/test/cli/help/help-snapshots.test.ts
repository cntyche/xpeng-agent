...
// terminal sizes. The default xpengagent tui command is excluded —
// `xpengagent --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `xpengagent --help` prints today;
...
// top-level help on `--help` and exits 1; not a real xpengagent command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("xpengagent CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ xpengagent }) =>
      Effect.gen(function* () {
        const topLevel = yield* xpengagent.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* xpengagent.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`xpengagent ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`xpengagent ${argv.join(" ")} --help`)
        }
