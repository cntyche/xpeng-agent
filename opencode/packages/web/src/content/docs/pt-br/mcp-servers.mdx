...
Você pode adicionar ferramentas externas ao xpengagent usando o _Model Context Protocol_, ou MCP. O xpengagent suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do xpengagent](https://xpengagent.cc.cd/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/xpengagent`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O xpengagent lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o xpengagent irá:
...
Se o servidor requer autenticação, o xpengagent solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `xpengagent mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o xpengagent armazenará os tokens de forma segura em `~/.local/share/xpengagent/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no xpengagent, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do xpengagent como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o xpengagent à sua conta do Sentry.
