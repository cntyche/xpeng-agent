...
U XPENGagent mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, XPENGagent koristi nasu `xpengagent` temu.
...
XPENGagent dolazi sa vise ugradenih tema.
...
- Zele da XPENGagent odgovara izgledu njihovog terminala
...
XPENGagent podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/xpengagent/themes/*.json` ili `$XDG_CONFIG_HOME/xpengagent/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.xpengagent/themes/*.json`
4. **Trenutni radni direktorij** - `./.xpengagent/themes/*.json`
