...
With XPENGagent you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, XPENGagent uses our own `xpengagent` theme.
...
XPENGagent comes with several built-in themes.
...
- Want XPENGagent to match their terminal's appearance
...
XPENGagent supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/xpengagent/themes/*.json` or `$XDG_CONFIG_HOME/xpengagent/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.xpengagent/themes/*.json`
4. **Current working directory** - Defined in `./.xpengagent/themes/*.json`
