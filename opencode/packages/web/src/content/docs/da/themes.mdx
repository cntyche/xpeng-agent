...
Med XPENGagent kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger XPENGagent vårt eget `xpengagent`-tema.
...
XPENGagent kommer med flere innebygde temaer.
...
- Vil at XPENGagent skal matche terminalens utseende
...
XPENGagent støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/xpengagent/themes/*.json` eller `$XDG_CONFIG_HOME/xpengagent/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.xpengagent/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.xpengagent/themes/*.json`
