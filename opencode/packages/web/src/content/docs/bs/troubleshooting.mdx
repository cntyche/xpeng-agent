...
Da biste otklonili probleme s XPENGagent, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\xpengagent\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `xpengagent --log-level DEBUG`.

---

## Pohrana

XPENGagent pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/xpengagent/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\xpengagent`
...
XPENGagent Desktop pokreće lokalni XPENGagent server (`xpengagent-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `XPENGagent` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/xpengagent/xpengagent.jsonc` (ili `~/.config/xpengagent/xpengagent.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/xpengagent/xpengagent.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\xpengagent\xpengagent.jsonc`
...
XPENGagent također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/xpengagent/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\xpengagent\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga XPENGagent mogao ponovo izgraditi.

1. Potpuno zatvorite XPENGagent Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/xpengagent`
- **Linux**: obrišite `~/.cache/xpengagent` (ili pokrenite `rm -rf ~/.cache/xpengagent`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\xpengagent`

3. Ponovo pokrenite XPENGagent Desktop.

---

### Rješavanje problema sa vezom na serveru

XPENGagent Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `xpengagent.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `XPENGAGENT_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `XPENGAGENT_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, XPENGagent Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa XPENGagent karakteristikama.

---

### Obavještenja se ne prikazuju

XPENGagent Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za XPENGagent u postavkama vašeg OS-a, i
...
1. Zatvorite XPENGagent Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka XPENGagent Desktop aplikacije):

- `xpengagent.settings.dat` (URL zadanog servera za desktop)
- `xpengagent.global.dat` i `xpengagent.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s XPENGagent:
...
Da saznate kojim modelima imate pristup, pokrenite `xpengagent models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\xpengagent`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. XPENGagent dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\xpengagent`

2. Ponovo pokrenite XPENGagent da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti XPENGagent da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
XPENGagent će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
