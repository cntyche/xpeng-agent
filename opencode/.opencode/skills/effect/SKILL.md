...
- Use `testEffect(...)` from `packages/xpengagent/test/lib/effect.ts` for tests that exercise Effect services, layers, runtime context, scoped resources, or platform integrations.
...
- Run tests from package directories such as `packages/xpengagent`; never run package tests from the repo root.
