...
import { LLMEvent, ToolFailure } from "@xpengagent/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@xpengagent/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("xpengagent"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("xpengagent") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("xpengagent") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, xpengagent, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored xpengagent auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("xpengagent") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("xpengagent"),
