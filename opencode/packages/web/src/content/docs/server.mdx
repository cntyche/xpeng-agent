---
title: Server
description: Interact with xpengagent server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `xpengagent serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an xpengagent client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `xpengagent.local` |
...
Set `XPENGAGENT_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `xpengagent`, or set `XPENGAGENT_SERVER_USERNAME` to override it. This applies to both `xpengagent serve` and `xpengagent web`.
...
When you run `xpengagent` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the xpengagent server to interact with xpengagent programmatically.
:::

This architecture lets xpengagent support multiple clients and allows you to interact with xpengagent programmatically.

You can run `xpengagent serve` to start a standalone server. If you have the
xpengagent TUI running, `xpengagent serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the XPENGagent [IDE](/docs/ide) plugins.
...
The xpengagent server exposes the following APIs.
