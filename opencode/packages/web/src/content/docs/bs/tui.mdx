---
title: TUI
description: Korištenje korisničkog interfejsa XPENGagent terminala.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

XPENGagent pruža interaktivni terminalski interfejs ili TUI za rad na vašim projektima sa LLM.

Pokretanje XPENGagent pokreće TUI za trenutni direktorij.
...
Kada koristite XPENGagent TUI, možete upisati `/` nakon čega slijedi ime komande da biste brzo izvršili radnje. na primjer:
...
Dodajte provajdera u XPENGagent. Omogućava vam da odaberete između dostupnih provajdera i dodate njihove API ključeve.
...
Izađite iz XPENGagent. _Aliases_: `/quit`, `/q`
...
Ovo je odvojeno od `xpengagent.json`, koji konfiguriše ponašanje servera/izvršavanja.
...
- `leader_timeout` - Kontroliše koliko dugo XPENGagent čeka nakon leader key. Podrazumijevano je `2000`.
...
Koristite `XPENGAGENT_TUI_CONFIG` da učitate prilagođenu putanju TUI konfiguracije.

### Attention

Attention omogućava da vas TUI obavijesti kada XPENGagent čeka odgovor, treba odobrenje dozvole, prijavi grešku sesije ili završi sesiju. Omogućite ga pomoću `attention.enabled`; ugrađeni događaji reproduciraju zvuk kada se dogode. Desktop obavještenja se šalju samo kada prozor terminala nije u fokusu i ne koriste se za subagent događaje.
...
- `sound_pack` - Sound pack ID koji se koristi. Podrazumijevano je `xpengagent.default`.
