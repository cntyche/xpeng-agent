...
Avec XPENGagent, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, XPENGagent utilise notre propre thème `xpengagent`.
...
XPENGagent est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que XPENGagent corresponde à l'apparence de leur terminal
...
XPENGagent prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/xpengagent/themes/*.json` ou `$XDG_CONFIG_HOME/xpengagent/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.xpengagent/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.xpengagent/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/xpengagent/themes
vim ~/.config/xpengagent/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .xpengagent/themes
vim .xpengagent/themes/my-theme.json
```
...
{
  "$schema": "https://xpengagent.cc.cd/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
