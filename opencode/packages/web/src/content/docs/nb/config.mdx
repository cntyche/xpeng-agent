---
title: Konfigurasjon
description: Ved å bruke xpengagent JSON-konfigurasjonen.
---

Du kan konfigurere XPENGagent ved å bruke en JSON konfigurasjonsfil.

---

## Format

XPENGagent støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "xpengagent"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/xpengagent`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/xpengagent/xpengagent.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`XPENGAGENT_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`xpengagent.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.xpengagent` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`XPENGAGENT_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.xpengagent` og `~/.config/xpengagent` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/xpengagent`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale XPENGagent-konfigurasjon i `~/.config/xpengagent/xpengagent.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `xpengagent.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når XPENGagent starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `XPENGAGENT_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `XPENGAGENT_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.xpengagent` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.xpengagent`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`xpengagent.cc.cd/config.json`**](https://xpengagent.cc.cd/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `xpengagent serve` og `xpengagent web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage XPENGagent-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `xpengagent.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i XPENGagent-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver XPENGagent å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/xpengagent/agents/` eller `.xpengagent/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil XPENGagent falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`xpengagent run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/xpengagent/commands/` eller `.xpengagent/commands/`. [Les mer her](/docs/commands).
...
XPENGagent vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater XPENGagent alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider XPENGagent med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.xpengagent/plugins/` eller `~/.config/xpengagent/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense XPENGagent til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
