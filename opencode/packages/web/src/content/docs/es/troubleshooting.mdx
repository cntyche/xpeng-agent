...
Para depurar problemas con XPENGagent, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\xpengagent\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `xpengagent --log-level DEBUG`.

---

## Almacenamiento

xpengagent almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/xpengagent/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\xpengagent`
...
XPENGagent Desktop ejecuta un servidor XPENGagent local (el sidecar `xpengagent-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `XPENGagent` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/xpengagent/xpengagent.jsonc` (o `~/.config/xpengagent/xpengagent.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/xpengagent/xpengagent.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\xpengagent\xpengagent.jsonc`
...
XPENGagent también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/xpengagent/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\xpengagent\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que XPENGagent pueda reconstruirlo.

1. Salga de XPENGagent Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/xpengagent`
- **Linux**: eliminar `~/.cache/xpengagent` (o ejecutar `rm -rf ~/.cache/xpengagent`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\xpengagent`

3. Reinicie el escritorio XPENGagent.

---

### Solucionar problemas de conexión del servidor

XPENGagent Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `xpengagent.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `XPENGAGENT_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `XPENGAGENT_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio XPENGagent requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de XPENGagent.

---

### Notificaciones que no se muestran

XPENGagent Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para XPENGagent en la configuración de su sistema operativo, y
...
1. Salga del escritorio XPENGagent.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio XPENGagent):

- `xpengagent.settings.dat` (URL del servidor predeterminado de escritorio)
- `xpengagent.global.dat` y `xpengagent.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con XPENGagent:
...
3. Asegúrese de tener la última versión con `xpengagent upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `xpengagent models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\xpengagent`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. xpengagent instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\xpengagent`

2. Reinicie xpengagent para reinstalar los paquetes del proveedor más recientes.

Esto obligará a xpengagent a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
xpengagent detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
