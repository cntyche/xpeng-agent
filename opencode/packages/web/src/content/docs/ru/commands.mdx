...
Создайте `.xpengagent/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию xpengagent или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем xpengagent [config](/docs/config):
...
- Глобальный: `~/.config/xpengagent/commands/`
- Для каждого проекта: `.xpengagent/commands/`
...
xpengagent включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
