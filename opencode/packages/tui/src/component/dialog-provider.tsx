...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@xpengagent/sdk/v2"
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in xpengagent.json to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
      ...
      description={
        {
          xpengagent: (
            <box gap={1}>
              <text fg={theme.textMuted}>
                XPENGagent Zen gives you access to all the best coding models at the cheapest prices with a single API
                key.
              </text>
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://xpengagent.cc.cd/zen</span> to get a key
              </text>
            </box>
          ),
          "xpengagent-go": (
            <box gap={1}>
              <text fg={theme.textMuted}>
                XPENGagent Go is a $10 per month subscription that provides reliable access to popular open coding models
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure it in xpengagent.json to use it.`,
          })
          dialog.clear()
          return
        }
