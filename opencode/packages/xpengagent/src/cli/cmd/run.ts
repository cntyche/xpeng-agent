import type { PermissionV1 } from "@xpengagent/core/v1/permission"
// CLI entry point for `xpengagent run`.
//
// Handles three modes:
...
//      xpengagent server and runs interactive mode against it.
//
...
import { createXpengagentClient, type OpencodeClient, type ToolPart } from "@xpengagent/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run xpengagent with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running xpengagent server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to XPENGAGENT_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to XPENGAGENT_SERVER_USERNAME or 'xpengagent')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const sdk = createXpengagentClient({
        baseUrl: "http://xpengagent.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
