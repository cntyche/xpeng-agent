---
title: Plugins
description: Escreva seus próprios plugins para estender o xpengagent.
---

Plugins permitem que você estenda o xpengagent conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do xpengagent.
...
- `.xpengagent/plugins/` - Plugins em nível de projeto
- `~/.config/xpengagent/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/xpengagent/node_modules/`.
...
1. Configuração global (`~/.config/xpengagent/xpengagent.json`)
2. Configuração do projeto (`xpengagent.json`)
3. Diretório de plugins global (`~/.config/xpengagent/plugins/`)
4. Diretório de plugins do projeto (`.xpengagent/plugins/`)
...
O xpengagent executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do xpengagent para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o xpengagent.
...
:::note
Se você estiver usando o aplicativo desktop xpengagent, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o xpengagent de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao xpengagent:
...
O helper `tool` cria uma ferramenta personalizada que o xpengagent pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o xpengagent junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://xpengagent.cc.cd/docs/sdk) para detalhes.
