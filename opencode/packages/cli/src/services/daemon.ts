...
import { createXpengagentClient } from "@xpengagent/sdk/v2/client"
import { ServerAuth } from "@xpengagent/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createXpengagentClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@xpengagent/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createXpengagentClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createXpengagentClient({ baseUrl: connection.url, headers: connection.headers })
    })
