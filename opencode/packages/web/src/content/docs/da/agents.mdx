...
Der er to typer agenter i XPENGagent; primære agenter og subagenter.
...
XPENGagent leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
XPENGagent leveres med tre indbyggede subagenter, **General**, **Explore** og **Scout**. Vi ser nærmere på dem nedenfor.

---

## Indbyggede

XPENGagent leveres med to indbyggede primære agenter og tre indbyggede subagenter.
...
En skrivebeskyttet agent til eksterne docs og research af dependencies. Brug denne, når du har brug for at klone et dependency-repository ind i XPENGagent's administrerede cache, inspicere kildekoden i et bibliotek eller krydstjekke lokal kode mod upstream-implementeringer uden at ændre dit workspace.
...
Konfigurer agent i din `xpengagent.json`-konfigurationsfil:
...
- Globalt: `~/.config/xpengagent/agents/`
- Pr. projekt: `.xpengagent/agents/`
...
Hvis ingen temperatur er angivet, bruger XPENGagent modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale XPENGagent-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din XPENGagent-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [XPENGagent Zen](/docs/zen), vil du bruge `xpengagent/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `xpengagent models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/anomalyco/xpengagent).
:::
