---
title: Konfiguracja
description: Ustawienia XPENGagent w pliku JSON.
---

Możesz dostosować XPENGagent za pomocą pliku konfiguracyjnego JSON.

---

## Format

XPENGagent obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "xpengagent"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/xpengagent`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/xpengagent/xpengagent.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `XPENGAGENT_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`xpengagent.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.xpengagent`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `XPENGAGENT_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.xpengagent` i `~/.config/xpengagent` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/xpengagent`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację XPENGagent w `~/.config/xpengagent/xpengagent.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/xpengagent/tui.json`.
...
Dodaj `xpengagent.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy XPENGagent się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `XPENGAGENT_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `XPENGAGENT_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.xpengagent` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.xpengagent`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`xpengagent.cc.cd/config.json`**](https://xpengagent.cc.cd/config.json).
...
Użyj `XPENGAGENT_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `xpengagent.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `xpengagent serve` i `xpengagent web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer XPENGagent.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `xpengagent.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie XPENGagent wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/xpengagent/agents/` lub `.xpengagent/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, XPENGagent powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`xpengagent run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/xpengagent/commands/` lub `.xpengagent/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
XPENGagent może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy XPENGagent nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie XPENGagent **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają XPENGagent o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.xpengagent/plugins/` lub `~/.config/xpengagent/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie XPENGagent tylko do określonych dostawców.
