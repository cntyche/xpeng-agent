...
test("rejects an update that did not install the desktop version", () => {
  ...
  expect(() => expectOpencodeVersion("1.14.35", "1.16.2")).toThrow(
    "XPENGagent update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating XPENGagent", () => {
  expect(
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.xpengagent/bin/xpengagent",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, xpengagentChecks: {} })
})
...
test("ignores stale background XPENGagent checks after removing a WSL server", async () => {
  persistedServers = []
  ...
  expect(controller.getState().xpengagentChecks).toEqual({})
})

test("ignores stale startup XPENGagent checks after removing a WSL server", async () => {
...
function testControllerOptions() {
  return {
    ...
    resolveOpencode: async () => {
      ...
      return "/home/me/.xpengagent/bin/xpengagent"
    },
  }
}
