...
Trim `packages/xpengagent/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/xpengagent/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/xpengagent`.
- `[x]` `bun typecheck` from `packages/xpengagent`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/xpengagent/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/xpengagent`.
- `[x]` `bun dev generate > /tmp/opencode-openapi.json` from `packages/xpengagent`.
...
- Edit `packages/xpengagent/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/opencode-openapi.json` from `packages/xpengagent`.
...
- Check source schemas in files like `packages/xpengagent/src/session/schema.ts`, `packages/xpengagent/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/xpengagent/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/xpengagent/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
