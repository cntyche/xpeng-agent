---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um XPENGagent zu erweitern.
---

Mit Plugins können Sie XPENGagent erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von XPENGagent zu ändern.
...
- `.xpengagent/plugins/` – Plugins auf Projektebene
- `~/.config/xpengagent/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/xpengagent/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/xpengagent/xpengagent.json`)
2. Projektkonfiguration (`xpengagent.json`)
3. Globales Plugin-Verzeichnis (`~/.config/xpengagent/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.xpengagent/plugins/`)
...
XPENGagent führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein XPENGagent-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie XPENGagent erweitern können.
...
:::note
Wenn Sie die XPENGagent-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass XPENGagent `.env`-Dateien liest:
...
Plugins können XPENGagent auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das XPENGagent aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für XPENGagent zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://xpengagent.cc.cd/docs/sdk).
