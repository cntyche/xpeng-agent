  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://xpengagent.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://xpengagent.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://xpengagent.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://xpengagent.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://xpengagent.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://xpengagent.example.com"),
      ),
    ).toEqual({
