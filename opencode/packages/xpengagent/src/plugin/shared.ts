...
export const DEPRECATED_PLUGIN_PACKAGES = ["xpengagent-openai-codex-auth", "xpengagent-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, xpengagentVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(xpengagentVersion) || semver.major(xpengagentVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.xpengagent
  if (typeof range !== "string") return
  if (!semver.satisfies(xpengagentVersion, range)) {
    throw new Error(`Plugin requires xpengagent ${range} but running ${xpengagentVersion}`)
  }
}
