...
Agent skills omogucavaju XPENGagent da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
XPENGagent pretrazuje ove lokacije:

- Konfiguracija projekta: `.xpengagent/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/xpengagent/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, XPENGagent ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.xpengagent/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/xpengagent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.xpengagent/skills/git-release/SKILL.md` ovako:
...
XPENGagent navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `xpengagent.json`:
...
**Za ugradene agente** (u `xpengagent.json`):
