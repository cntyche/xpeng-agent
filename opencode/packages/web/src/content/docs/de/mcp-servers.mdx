...
Sie können externe Tools zu XPENGagent hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. XPENGagent unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [XPENGagent Config](https://xpengagent.cc.cd/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/xpengagent`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
XPENGagent übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird XPENGagent Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von XPENGagent beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `xpengagent mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert XPENGagent die Token sicher in `~/.local/share/xpengagent/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in XPENGagent verfügbar. Sie können sie also wie jedes andere Tool über die XPENGagent-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und XPENGagent mit Ihrem Sentry-Konto zu verbinden.
