---
title: Sunucu
description: xpengagent sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`xpengagent serve` komutu, xpengagent istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `xpengagent.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `XPENGAGENT_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `xpengagent` değeridir; değiştirmek isterseniz `XPENGAGENT_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `xpengagent serve` hem de `xpengagent web` için geçerlidir.
...
`xpengagent` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
xpengagent ile programatik etkileşim için xpengagent sunucusunu kullanın.
:::

Bu mimari, xpengagent'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`xpengagent serve` ile bağımsız bir sunucu başlatabilirsiniz. xpengagent TUI açıksa, `xpengagent serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum xpengagent [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
xpengagent sunucusu aşağıdaki API'leri sunar.
