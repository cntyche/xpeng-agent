---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w xpengagent.
---

import config from "../../../../config.mjs"
export const console = config.console

xpengagent używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji xpengagent.
...
w `~/.local/share/xpengagent/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim xpengagent
config.
...
XPENGagent Zen to lista modeli dostarczonych przez zespół xpengagent, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z xpengagent. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od XPENGagent Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `XPENGagent Zen` i przejdź do [xpengagent.cc.cd/auth](https://xpengagent.cc.cd/zen).
...
Działa jak każdy inny dostawca w xpengagent i jest całkowicie opcjonalny w użyciu.

---

## XPENGagent Go

XPENGagent Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół xpengagent, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z xpengagent.

1. Uruchom polecenie `/connect` w TUI, wybierz `XPENGagent Go` i przejdź do [xpengagent.cc.cd/auth](https://xpengagent.cc.cd/zen).
...
Aby używać Amazon Bedrock z xpengagent:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania xpengagent:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `xpengagent.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w xpengagent nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować xpengagent do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod xpengagent działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji xpengagent.
...
xpengagent używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować xpengagent
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`xpengagent.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [xpengagent-gitlab-auth](https://www.npmjs.com/package/xpengagent-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem xpengagent:
...
Aby używać Google Vertex AI z xpengagent:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania xpengagent.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą xpengagent, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [xpengagent-helicone-session](https://github.com/H2Shami/xpengagent-helicone-session), aby automatycznie rejestrować każdą konwersację xpengagent jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja xpengagent będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować xpengagent tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować xpengagent tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować xpengagent tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem xpengagent. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/xpengagent).
:::
...
Aby korzystać z Ollama Cloud z xpengagent:
...
3. Skopiuj klucz API do użycia w xpengagent.
...
6. **Ważne**: Przed użyciem modeli chmurowych w xpengagent musisz pobrać informacje o modelu lokalnie:
...
XPENGagent Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół xpengagent. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>XPENGagent Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **XPENGagent Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API xpengagent.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji xpengagent.
...
5. Możesz je także dostosować za pomocą konfiguracji xpengagent. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z xpengagent:
...
Możesz także dostosować modele za pomocą konfiguracji xpengagent. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z xpengagent. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `xpengagent.json` w katalogu projektu:
...
Pola `limit` pozwalają xpengagent zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `xpengagent auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację xpengagent i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji xpengagent.
