...
Mit XPENGagent kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet XPENGagent das `xpengagent`-Theme.
...
XPENGagent liefert mehrere eingebaute Themes mit.
...
- XPENGagent optisch ans Terminal anpassen wollen
...
XPENGagent unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/xpengagent/themes/*.json` oder `$XDG_CONFIG_HOME/xpengagent/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.xpengagent/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.xpengagent/themes/*.json`
