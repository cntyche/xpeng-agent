...
export async function spawnWslSidecar(
  ...
  const xpengagent = await resolveWslOpencode(distro)
  if (!xpengagent) throw new Error(`XPENGagent is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "xpengagent"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export XPENGAGENT_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export XPENGAGENT_CLIENT=desktop",
    `export XPENGAGENT_SERVER_USERNAME=${shellEscape(username)}`,
    `export XPENGAGENT_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(xpengagent)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
