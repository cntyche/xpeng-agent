...
import type { Message, OpencodeClient, SessionMessageResponse } from "@xpengagent/sdk/v2"
...
export const AuthMethodID = "xpengagent-login"
...
export class Service extends Context.Service<Service, Interface>()("@xpengagent/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `xpengagent auth login` in the terminal",
      name: "Login with xpengagent",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, xpengagent provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const xpengagentProvider = providers[ProviderV2.ID.make("xpengagent")]
  const xpengagentModel = xpengagentProvider ? Provider.sort(Object.values(xpengagentProvider.models))[0] : undefined
  if (xpengagentProvider && xpengagentModel) return { providerID: xpengagentProvider.id, modelID: xpengagentModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "XPENGagent service failure", service })
}
