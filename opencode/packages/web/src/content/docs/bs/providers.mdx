---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u XPENGagent.
---

import config from "../../../../config.mjs"
export const console = config.console

XPENGagent koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj XPENGagent konfiguraciji.
...
u `~/.local/share/xpengagent/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem XPENGagent
config.
...
XPENGagent Zen je lista modela koje je obezbedio XPENGagent tim koji su bili
testirano i potvrđeno da dobro radi sa XPENGagent. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa XPENGagent Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite xpengagent i idite na [xpengagent.cc.cd/auth](https://xpengagent.cc.cd/auth).
...
Radi kao i svaki drugi provajder u XPENGagent i potpuno je opcionalan za korištenje.

---

## XPENGagent Go

XPENGagent Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža XPENGagent tim i koji su testirani i verificirani da dobro rade s XPENGagent-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `XPENGagent Go` i idite na [xpengagent.cc.cd/auth](https://xpengagent.cc.cd/zen).
...
Da biste koristili Amazon Bedrock s XPENGagent:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete xpengagent:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `xpengagent.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u XPENGagent.
:::
...
Možete konfigurirati xpengagent za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi xpengagent ispravno radio.
   :::
...
Također možete dodati modele kroz svoju xpengagent konfiguraciju.
...
da pohrani token u xpengagent auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
XPENGagent koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate XPENGagent
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`xpengagent.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [xpengagent-gitlab-auth](https://www.npmjs.com/package/xpengagent-gitlab-auth).

##### Konfiguracija

Prilagodite putem `xpengagent.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s xpengagent:
...
Za korištenje Google Vertex AI s XPENGagent:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće xpengagent.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem xpengagenta, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [xpengagent-helicone-session](https://github.com/H2Shami/xpengagent-helicone-session) da automatski prijavite svaki XPENGagent razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki XPENGagent razgovor naveden kao zasebna sesija.
...
Možete konfigurirati xpengagent za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati xpengagent za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati xpengagent za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za XPENGagent. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/xpengagent) za detalje.
:::
...
Da biste koristili Ollama Cloud s XPENGagent:
...
3. Kopirajte API ključ za korištenje u XPENGagent.
...
6. **Važno**: Prije upotrebe modela oblaka u XPENGagent, morate lokalno povući informacije o modelu:
...
XPENGagent Zen je lista testiranih i verifikovanih modela koju je obezbedio XPENGagent tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>XPENGagent Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **XPENGagent Zen**.

```txt
   /connect
```

3. Unesite svoj XPENGagent API ključ.
...
Također možete dodati dodatne modele putem vaše xpengagent konfiguracije.
...
5. Također ih možete prilagoditi putem vaše xpengagent konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju xpengagent konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s xpengagent-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `xpengagent.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju XPENGagent da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `xpengagent auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite XPENGagent konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj xpengagent konfiguraciji.
