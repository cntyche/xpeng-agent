...
Навыки агента позволяют xpengagent обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
xpengagent выполняет поиск в следующих местах:

- Конфигурация проекта: `.xpengagent/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/xpengagent/skills/<name>/SKILL.md`.
...
Для локальных путей проекта xpengagent переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
Он загружает все соответствующие `skills/*/SKILL.md` в `.xpengagent/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/xpengagent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.xpengagent/skills/git-release/SKILL.md` следующим образом:
...
xpengagent перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `xpengagent.json`:
...
**Для встроенных агентов** (в формате `xpengagent.json`):
