---
title: Regeln
description: Lege eigene Anweisungen fuer xpengagent fest.
---

Du kannst xpengagent ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in xpengagent den Befehl `/init` aus.
...
So kann xpengagent sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht xpengagent sie zu erweitern.
...
xpengagent kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/xpengagent/AGENTS.md` speichern.
Diese gelten in allen xpengagent-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt XPENGagent dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/xpengagent/AGENTS.md` existiert)
...
Beim Start sucht xpengagent in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/xpengagent/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `xpengagent.json` oder in `~/.config/xpengagent/xpengagent.json` angeben.
...
xpengagent parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit xpengagent.json

Empfohlen ist das `instructions`-Feld in der `xpengagent.json`:
...
Du kannst xpengagent explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `xpengagent.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
