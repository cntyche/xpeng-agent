...
Dzięki xpengagent możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie xpengagent używa naszego własnego motywu `xpengagent`.
...
xpengagent ma kilka wbudowanych motywów.
...
- Chcą, aby XPENGagent pasował do wyglądu ich terminala
...
xpengagent obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/xpengagent/themes/*.json` lub `$XDG_CONFIG_HOME/xpengagent/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.xpengagent/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.xpengagent/themes/*.json`
