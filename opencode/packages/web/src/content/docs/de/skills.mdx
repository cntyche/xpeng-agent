...
Agent Skills erlauben XPENGagent, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
XPENGagent sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft XPENGagent vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.xpengagent/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/xpengagent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.xpengagent/skills/git-release/SKILL.md` zum Beispiel so:
...
XPENGagent listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `xpengagent.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `xpengagent.json`):
