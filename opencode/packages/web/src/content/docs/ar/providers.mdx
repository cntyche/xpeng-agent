---
title: المزوّدون
description: استخدام أي موفّر LLM في XPENGagent.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم XPENGagent ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات XPENGagent.
...
في `~/.local/share/xpengagent/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات XPENGagent.
...
XPENGagent Zen هي قائمة نماذج يوفّرها فريق XPENGagent وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع XPENGagent. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع XPENGagent Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر xpengagent، ثم انتقل إلى [xpengagent.cc.cd/auth](https://xpengagent.cc.cd/auth).
...
يعمل مثل أي مزوّد آخر في XPENGagent واستخدامه اختياري بالكامل.

---

## XPENGagent Go

XPENGagent Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق XPENGagent، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع XPENGagent.

1. شغّل الأمر `/connect` في TUI، واختر `XPENGagent Go`، ثم انتقل إلى [xpengagent.cc.cd/auth](https://xpengagent.cc.cd/zen).
...
لاستخدام Amazon Bedrock مع XPENGagent:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل xpengagent:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `xpengagent.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل XPENGagent غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين xpengagent لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل xpengagent بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات xpengagent.
...
تخزين الرمز في مساحة تخزين مصادقة xpengagent.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم XPENGagent نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر XPENGagent
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`xpengagent.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [xpengagent-gitlab-auth](https://www.npmjs.com/package/xpengagent-gitlab-auth).

##### التهيئة

خصّص عبر `xpengagent.json`:
...
لاستخدام اشتراك GitHub Copilot مع xpengagent:
...
لاستخدام Google Vertex AI مع XPENGagent:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر xpengagent، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [xpengagent-helicone-session](https://github.com/H2Shami/xpengagent-helicone-session) لتسجيل كل محادثة XPENGagent تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة XPENGagent مدرجة كجلسة مستقلة.
...
يمكنك ضبط xpengagent لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط xpengagent لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط xpengagent لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع XPENGagent. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/xpengagent) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع XPENGagent:
...
3. انسخ مفتاح API لاستخدامه في XPENGagent.
...
6. **مهم**: قبل استخدام نماذج السحابة في XPENGagent، يجب سحب معلومات النموذج محليا:
...
XPENGagent Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق XPENGagent. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>XPENGagent Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **XPENGagent Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ XPENGagent.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات xpengagent.
...
5. يمكنك أيضا تخصيصها عبر إعدادات xpengagent. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات xpengagent. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع xpengagent. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `xpengagent.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ XPENGagent بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `xpengagent auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات xpengagent وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات xpengagent.
