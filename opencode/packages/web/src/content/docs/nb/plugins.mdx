...
description: Skriv dine egne programtillegg for å utvide XPENGagent.
---

Plugins lar deg utvide XPENGagent ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til XPENGagent.
...
- `.xpengagent/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/xpengagent/node_modules/`.
...
1. Global konfigurasjon (`~/.config/xpengagent/xpengagent.json`)
2. Prosjektkonfigurasjon (`xpengagent.json`)
3. Global plugin-katalog (`~/.config/xpengagent/plugins/`)
4. Prosjektpluginkatalog (`.xpengagent/plugins/`)
...
XPENGagent kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En XPENGagent SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide XPENGagent.
...
:::note
Hvis du bruker XPENGagent-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre XPENGagent fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til XPENGagent:
...
`tool`-hjelperen lager et tilpasset verktøy som XPENGagent kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for XPENGagent sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://xpengagent.cc.cd/docs/sdk) for detaljer.
