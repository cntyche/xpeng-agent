...
Puede agregar herramientas externas a XPENGagent utilizando el _Model Context Protocol_ o MCP. XPENGagent admite servidores locales y remotos.
...
Puede definir servidores MCP en su [XPENGagent Config](https://xpengagent.cc.cd/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/xpengagent`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
XPENGagent maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, XPENGagent:
...
Si el servidor requiere autenticación, XPENGagent le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `xpengagent mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, XPENGagent almacenará los tokens de forma segura en `~/.local/share/xpengagent/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en XPENGagent, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración XPENGagent como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar XPENGagent a su cuenta Sentry.
