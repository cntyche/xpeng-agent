---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i XPENGagent.
---

import config from "../../../../config.mjs"
export const console = config.console

XPENGagent bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i XPENGagent-konfigurasjonen.
...
i `~/.local/share/xpengagent/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i XPENGagent
konfig.
...
XPENGagent Zen er en liste over modeller levert av XPENGagent-teamet som har vært
testet og verifisert for å fungere godt med XPENGagent. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med XPENGagent Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `XPENGagent Zen` og gå til [xpengagent.cc.cd/auth](https://xpengagent.cc.cd/zen).
...
Det fungerer som alle andre leverandører i XPENGagent og er helt valgfritt å bruke.

---

## XPENGagent Go

XPENGagent Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av XPENGagent-teamet som har vært
testet og verifisert for å fungere godt med XPENGagent.

1. Kjør kommandoen `/connect` i TUI, velg `XPENGagent Go`, og gå til [xpengagent.cc.cd/auth](https://xpengagent.cc.cd/zen).
...
Slik bruker du Amazon Bedrock med XPENGagent:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører XPENGagent:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `xpengagent.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i xpengagent støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere xpengagent til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at XPENGagent skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom XPENGagent-konfigurasjonen.
...
for å lagre token i XPENGagent auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
XPENGagent bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse XPENGagent
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`xpengagent.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [xpengagent-gitlab-auth](https://www.npmjs.com/package/xpengagent-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `xpengagent.json`:
...
Slik bruker du GitHub Copilot-abonnementet med XPENGagent:
...
Slik bruker du Google Vertex AI med XPENGagent:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører XPENGagent.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom XPENGagent, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [xpengagent-helicone-session](https://github.com/H2Shami/xpengagent-helicone-session) plugin for automatisk å logge hver XPENGagent-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver XPENGagent-samtale oppført som en separat økt.
...
Du kan konfigurere XPENGagent for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere XPENGagent for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere XPENGagent for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for XPENGagent. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/xpengagent) for detaljer.
:::
...
Slik bruker du Ollama Cloud med XPENGagent:
...
3. Kopier API-nøkkelen for bruk i XPENGagent.
...
6. **Viktig**: Før du bruker skymodeller i XPENGagent, må du hente modellinformasjonen lokalt:
...
XPENGagent Zen er en liste over testede og verifiserte modeller levert av XPENGagent-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>XPENGagent Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **XPENGagent Zen**.

   ```txt
   /connect
   ```

3. Skriv inn XPENGagent API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom XPENGagent-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom XPENGagent-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med XPENGagent:
...
Du kan også tilpasse modeller gjennom XPENGagent-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med XPENGagent. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `xpengagent.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar XPENGagent forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `xpengagent auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk XPENGagent-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i XPENGagent-konfigurasjonen.
