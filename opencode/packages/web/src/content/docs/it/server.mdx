---
title: Server
description: Interagisci con il server xpengagent via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `xpengagent serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client xpengagent.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `xpengagent.local` |
...
Imposta `XPENGAGENT_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `xpengagent`, oppure imposta `XPENGAGENT_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `xpengagent serve` sia per `xpengagent web`.
...
Quando esegui `xpengagent` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server xpengagent per interagire con xpengagent in modo programmatico.
:::

Questa architettura permette a xpengagent di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `xpengagent serve` per avviare un server standalone. Se la TUI di xpengagent e gia in esecuzione, `xpengagent serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di XPENGagent.
...
Il server xpengagent espone le seguenti API.
