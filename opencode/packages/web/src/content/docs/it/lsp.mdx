---
title: Server LSP
description: XPENGagent si integra con i tuoi server LSP.
---

XPENGagent può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

XPENGagent include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `XPENGAGENT_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e xpengagent apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di xpengagent.
