...
You can add external tools to XPENGagent using the _Model Context Protocol_, or MCP. XPENGagent supports both local and remote servers.
...
You can define MCP servers in your [XPENGagent Config](https://xpengagent.cc.cd/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/xpengagent` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
XPENGagent automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, XPENGagent will:
...
If the server requires authentication, XPENGagent will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `xpengagent mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, XPENGagent will store the tokens securely in `~/.local/share/xpengagent/mcp-auth.json`.
...
Your MCPs are available as tools in XPENGagent, alongside built-in tools. So you can manage them through the XPENGagent config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect XPENGagent to your Sentry account.
