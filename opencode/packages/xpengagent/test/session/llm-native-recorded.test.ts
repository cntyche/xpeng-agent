...
import { SessionV1 } from "@xpengagent/core/v1/session"
import { FSUtil } from "@xpengagent/core/fs-util"
import { ModelsDev } from "@xpengagent/core/models-dev"
import { HttpRecorder } from "@xpengagent/http-recorder"
import { HttpRecorderInternal } from "@xpengagent/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@xpengagent/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@xpengagent/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.xpengagent.cc.cd/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.XPENGAGENT_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["xpengagent", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("XPENGAGENT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("XPENGAGENT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["xpengagent", "native", "oauth", "tool-loop"],
  ...
  {
    id: "xpengagent-proxy",
    name: "XPENGagent proxy",
    providerID: ProviderV2.ID.xpengagent,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["xpengagent", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.XPENGAGENT_RECORD_CONSOLE_TOKEN && process.env.XPENGAGENT_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.xpengagent,
        name: "XPENGagent Zen",
        env: ["XPENGAGENT_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.XPENGAGENT_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.XPENGAGENT_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.XPENGAGENT_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["xpengagent", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("XPENGAGENT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("XPENGAGENT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("XPENGAGENT_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set XPENGAGENT_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the xpengagent LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "xpengagent.json"),
      JSON.stringify({ $schema: "https://xpengagent.cc.cd/config.json", ...scenario.config(model) }),
    ),
  )
