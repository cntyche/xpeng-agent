...
Det er to typer agenter i XPENGagent; primære agenter og underagenter.
...
XPENGagent kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
XPENGagent kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

XPENGagent kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `xpengagent.json`:
...
- Per prosjekt: `.xpengagent/agents/`
...
Hvis ingen temperatur er spesifisert, bruker XPENGagent modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale XPENGagent-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i XPENGagent-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [XPENGagent Zen](/docs/zen), vil du bruke `xpengagent/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `xpengagent models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/xpengagent).
:::
