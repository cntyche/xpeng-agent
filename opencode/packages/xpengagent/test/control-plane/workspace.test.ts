...
import { EventSequenceTable } from "@xpengagent/core/event/sql"
...
import { WorkspaceV2 } from "@xpengagent/core/workspace"
import { WorkspaceTable } from "@xpengagent/core/control-plane/workspace.sql"
...
import { Ripgrep } from "@xpengagent/core/ripgrep"

const originalEnv = {
  XPENGAGENT_AUTH_CONTENT: process.env.XPENGAGENT_AUTH_CONTENT,
  XPENGAGENT_EXPERIMENTAL_WORKSPACES: process.env.XPENGAGENT_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.XPENGAGENT_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@xpengagent.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.XPENGAGENT_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=xpengagent-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.XPENGAGENT_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.XPENGAGENT_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.XPENGAGENT_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=xpengagent-test")
