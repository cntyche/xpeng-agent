...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@xpengagent/acme")).toBe("@xpengagent/acme")
    expect(Npm.sanitize("@xpengagent/acme@1.0.0")).toBe("@xpengagent/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/xpengagent/acme.git"
    const expected = win ? "acme@git+https_//github.com/xpengagent/acme.git" : spec
