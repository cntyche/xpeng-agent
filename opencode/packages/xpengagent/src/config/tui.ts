...
import { isRecord } from "@xpengagent/tui/util/record"
...
import { TuiKeybind } from "@xpengagent/tui/config/keybind"
...
import { TuiConfig } from "@xpengagent/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@xpengagent/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old xpengagent.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.xpengagent`
  // folders between cwd and home, and XPENGAGENT_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.XPENGAGENT_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit XPENGAGENT_TUI_CONFIG override, if set.
  if (Flag.XPENGAGENT_TUI_CONFIG) {
    const configFile = Flag.XPENGAGENT_TUI_CONFIG
  ...
  // 4. `.xpengagent` directories (and XPENGAGENT_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".xpengagent") || dir === Flag.XPENGAGENT_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".xpengagent") && dir !== Flag.XPENGAGENT_CONFIG_DIR) continue
