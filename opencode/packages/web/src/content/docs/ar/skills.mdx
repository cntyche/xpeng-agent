...
تتيح مهارات الوكيل لـ XPENGagent اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث XPENGagent في هذه المواقع:

- إعدادات المشروع: `.xpengagent/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/xpengagent/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج XPENGagent صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.xpengagent/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/xpengagent/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.xpengagent/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض XPENGagent المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `xpengagent.json`:
...
**للوكلاء المدمجين** (في `xpengagent.json`):
