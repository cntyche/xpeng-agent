...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.xpengagent.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `xpengagent.settings.dat` is special: it maps to the `xpengagent.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"xpengagent.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // xpengagent.settings.dat → the electron settings store ("xpengagent.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "xpengagent.global.dat").
  const storeName = filename === "xpengagent.settings.dat" ? "xpengagent.settings" : filename
