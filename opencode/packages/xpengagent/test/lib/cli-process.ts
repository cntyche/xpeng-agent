// Subprocess test harness for the xpengagent CLI. Spawns the real binary against
...
//
// Configuration flows through xpengagent's built-in test affordances:
//   - XPENGAGENT_CONFIG_CONTENT      : provider config inline, no files to find
//   - XPENGAGENT_TEST_HOME           : pins os.homedir() → tmpdir
//   - XPENGAGENT_DISABLE_PROJECT_CONFIG : skip walking up for xpengagent.json
//   - XPENGAGENT_PURE                : skip external plugin discovery + install
//   - XPENGAGENT_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `xpengagent.run` is fully wired. The shape supports adding more
// builders (`xpengagent.serve(opts)`, `xpengagent.acp(opts)`, `xpengagent.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of OpencodeCli.
import { test, type TestOptions } from "bun:test"
import { FSUtil } from "@xpengagent/core/fs-util"
import { AppProcess } from "@xpengagent/core/process"
...
const xpengagentRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(xpengagentRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    XPENGAGENT_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    XPENGAGENT_CONFIG_CONTENT: configJson,
    XPENGAGENT_DISABLE_PROJECT_CONFIG: "1",
    XPENGAGENT_PURE: "1",
    XPENGAGENT_DISABLE_AUTOUPDATE: "1",
    XPENGAGENT_DISABLE_AUTOCOMPACT: "1",
    XPENGAGENT_DISABLE_MODELS_FETCH: "1",
    XPENGAGENT_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `xpengagent run`. New flags should
...
// `xpengagent serve` is a long-lived process — it never exits on its own.
...
// `xpengagent acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `xpengagent serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `xpengagent acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly xpengagent: OpencodeCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("xpengagent.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("xpengagent.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "xpengagent server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `xpengagent serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("xpengagent.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const xpengagent: OpencodeCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, xpengagent })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `xpengagent.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "xpengagent") {
...
// every call site. Use this for any test that needs the xpengagent CLI fixture.
//
...
// resources (e.g. `xpengagent.serve`) without an extra `Effect.scoped` wrapper —
