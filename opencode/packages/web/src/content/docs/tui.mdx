---
title: TUI
description: Using the XPENGagent terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

XPENGagent provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running XPENGagent starts the TUI for the current directory.
...
When using the XPENGagent TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to XPENGagent. Allows you to select from available providers and add their API keys.
...
Exit XPENGagent. _Aliases_: `/quit`, `/q`
...
This is separate from `xpengagent.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long XPENGagent waits after the leader key. Defaults to `2000`.
...
Use `XPENGAGENT_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `xpengagent.default`.
