...
Las herramientas permiten que LLM realice acciones en su código base. XPENGagent viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en XPENGagent.
...
:::note
Esta herramienta solo está disponible cuando `XPENGAGENT_EXPERIMENTAL_LSP_TOOL=true` (o `XPENGAGENT_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor XPENGagent o cuando la variable de entorno `XPENGAGENT_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar XPENGagent:

```bash
XPENGAGENT_ENABLE_EXA=1 xpengagent
```
