...
import { isZedTerminal, offsetToPosition, resolveZedDbPath, resolveZedSelection } from "@xpengagent/tui/editor-zed"
...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.XPENGAGENT_ZED_DB
  process.env.XPENGAGENT_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.XPENGAGENT_ZED_DB
    else process.env.XPENGAGENT_ZED_DB = previous
    home.mockRestore()
  }
})
