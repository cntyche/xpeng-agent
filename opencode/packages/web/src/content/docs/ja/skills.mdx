...
エージェントスキルにより、XPENGagent はリポジトリまたはホームディレクトリから再利用可能な命令を検出できます。
...
XPENGagent は次の場所を検索します。

- プロジェクト設定: `.xpengagent/skills/<name>/SKILL.md`
- グローバル設定: `~/.config/xpengagent/skills/<name>/SKILL.md`
...
プロジェクトのローカルパスの場合、XPENGagent は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
途中で、一致する `skills/*/SKILL.md` を `.xpengagent/` に読み込み、一致する `.claude/skills/*/SKILL.md` または `.agents/skills/*/SKILL.md` を読み込みます。

グローバル定義は、`~/.config/xpengagent/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md`、および `~/.agents/skills/*/SKILL.md` からもロードされます。
...
次のように `.xpengagent/skills/git-release/SKILL.md` を作成します。
...
XPENGagent では、`skill` ツールの説明に利用可能なスキルがリストされています。
...
`xpengagent.json` のパターンベースの権限を使用して、エージェントがアクセスできるスキルを制御します。
...
**組み込みエージェントの場合** (`xpengagent.json` 内):
