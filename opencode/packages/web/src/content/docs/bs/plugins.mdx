---
title: Dodaci
description: Napišite vlastite dodatke za proširenje XPENGagent.
---

Dodaci vam omogućavaju da proširite XPENGagent spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja XPENGagent.
...
- `.xpengagent/plugins/` - Dodaci na nivou projekta
- `~/.config/xpengagent/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/xpengagent/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/xpengagent/xpengagent.json`)
2. Konfiguracija projekta (`xpengagent.json`)
3. Globalni direktorij dodataka (`~/.config/xpengagent/plugins/`)
4. Direktorij dodataka projekta (`.xpengagent/plugins/`)
...
XPENGagent pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje XPENGagent.
...
Spriječite xpengagent da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u xpengagent:
...
Pomoćnik `tool` kreira prilagođeni alat koji xpengagent može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za xpengagent zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://xpengagent.cc.cd/docs/sdk) za detalje.
