...
import { ConfigProvider } from "@xpengagent/core/config/provider"
import { ConfigMigrateV1 } from "@xpengagent/core/v1/config/migrate"
...
describe("Config", () => {
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(
                path.join(tmp.path, "xpengagent.json"),
                JSON.stringify({ $schema: "middle", providers: { middle: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "xpengagent.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "xpengagent.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(path.join(tmp.path, "xpengagent.json"), "{ invalid"),
              fs.writeFile(path.join(tmp.path, "xpengagent.jsonc"), JSON.stringify({ providers: { invalid: true } })),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "xpengagent.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .xpengagent configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".xpengagent"), { recursive: true })
            await fs.mkdir(path.join(directory, ".xpengagent"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "xpengagent.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "xpengagent.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "xpengagent.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "xpengagent.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "config.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".xpengagent", "xpengagent.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".xpengagent", "xpengagent.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".xpengagent")),
              AbsolutePath.make(path.join(directory, ".xpengagent")),
            ])
