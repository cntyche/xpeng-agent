...
import { XPENGagentHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for xpengagent serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(XPENGagentHttpApi)
...
export const PublicApi = XPENGagentHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "xpengagent",
    version: "1.0.0",
    description: "xpengagent api",
