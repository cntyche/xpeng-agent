---
title: Sağlayıcılar
description: xpengagent'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

xpengagent, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı xpengagent yapılandırmanızda yapılandırın.
...
`~/.local/share/xpengagent/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları xpengagent'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
XPENGagent Zen, xpengagent ekibi tarafından sağlanan modellerin bir listesidir.
xpengagent ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz XPENGagent Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, xpengagent'u seçin ve [xpengagent.cc.cd/auth](https://xpengagent.cc.cd/auth)'ye gidin.
...
xpengagent'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## XPENGagent Go

XPENGagent Go, xpengagent ile iyi çalıştığı test edilmiş ve doğrulanmış, xpengagent ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `XPENGagent Go`'yu seçin ve [xpengagent.cc.cd/auth](https://xpengagent.cc.cd/zen) adresine gidin.
...
Amazon Bedrock'u xpengagent ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   xpengagent'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `xpengagent.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi xpengagent'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
xpengagent'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   xpengagent'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
xpengagent, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. xpengagent'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`xpengagent.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [xpengagent-gitlab-auth](https://www.npmjs.com/package/xpengagent-gitlab-auth) ana sayfasında.

##### Yapılandırma

`xpengagent.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi xpengagent'la kullanmak için:
...
Google Vertex AI'yi xpengagent ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları xpengagent çalıştırılırken ayarlayın.
...
Helicone'un xpengagent aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her xpengagent görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [xpengagent-helicone-session](https://github.com/H2Shami/xpengagent-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her xpengagent görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
xpengagent'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla xpengagent'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla xpengagent'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini xpengagent için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/xpengagent)'a bakın.
:::
...
Ollama Cloud'u xpengagent ile kullanmak için:
...
3. xpengagent'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: xpengagent'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
XPENGagent Zen, xpengagent ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>XPENGagent Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **XPENGagent Zen**'i arayın.

   ```txt
   /connect
   ```

3. xpengagent API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   xpengagent yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca xpengagent yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri xpengagent yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı xpengagent ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `xpengagent.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları xpengagent'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `xpengagent auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için xpengagent yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, xpengagent yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
