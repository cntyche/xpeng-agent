...
import type { LLMClientShape } from "@xpengagent/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("xpengagent"))
    return { type: "unsupported", reason: "provider is not openai, xpengagent, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @xpengagent/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains xpengagent-owned. The native runtime only adapts
      // the @xpengagent/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
