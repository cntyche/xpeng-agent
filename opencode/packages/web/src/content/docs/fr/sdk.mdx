---
title: SDK
description: Client JS de type sécurisé pour le serveur xpengagent.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK xpengagent JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler xpengagent par programme.
...
Créez une instance de xpengagent :
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `xpengagent.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createOpencode } from "@xpengagent/sdk"

const xpengagent = await createOpencode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${xpengagent.server.url}`)

xpengagent.server.close()
```

## Client uniquement

Si vous disposez déjà d'une instance en cours d'exécution de xpengagent, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@xpengagent/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*xpengagent" },
})
