...
import type { HostPluginApi } from "@xpengagent/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createXpengagentClient({
  ...
  let selected = opts.theme?.selected ?? "xpengagent"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "xpengagent.default",
