...
const reg = process.env.REGISTRY ?? "ghcr.io/cntyche"
...
const setup = async () => {
  ...
  if (list.includes("xpengagent")) {
    await $`docker buildx use xpengagent`
    return
  }
  await $`docker buildx create --name xpengagent --use`
}
