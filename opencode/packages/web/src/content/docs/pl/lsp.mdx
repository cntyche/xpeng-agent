---
title: Serwery LSP
description: XPENGagent integruje się z serwerami LSP.
---

XPENGagent może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

XPENGagent posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `XPENGAGENT_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i XPENGagent otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji XPENGagent.
