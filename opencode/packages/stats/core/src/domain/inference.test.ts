...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve xpengagent route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "xpengagent")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "xpengagent")).toBe("openai")
    expect(statProvider("big-pickle", "", "xpengagent")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "xpengagent"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep xpengagent as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "xpengagent"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "xpengagent"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep xpengagent or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "xpengagent"), country: "US" })).toMatchObject([
