...
describe("xpengagent acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ xpengagent }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ xpengagent }))
        ...
        expect(initialized.agentInfo?.name).toBe("XPENGagent")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ xpengagent }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ xpengagent })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("xpengagent-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "xpengagent-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.XPENGAGENT_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ xpengagent }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("xpengagent-login")
