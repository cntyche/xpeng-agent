...
import type { UpdaterState } from "@xpengagent/app/updater"
...
import "./styles.css"
import { Splash } from "@xpengagent/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.XPENGAGENT_CHANNEL === "prod" &&
...
const deepLinkEvent = "xpengagent:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__XPENGAGENT__ ??= {}
  const pending = window.__XPENGAGENT__.deepLinks ?? []
  window.__XPENGAGENT__.deepLinks = [...pending, ...urls]
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://xpengagent.cc.cd/favicon-96x96-v3.png",
      })
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("xpengagent.global.dat").getItem("language")
