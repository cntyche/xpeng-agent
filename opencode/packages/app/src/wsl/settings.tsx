...
import { Tag } from "@xpengagent/ui/v2/badge-v2"
import { ButtonV2 } from "@xpengagent/ui/v2/button-v2"
import { Dialog } from "@xpengagent/ui/v2/dialog-v2"
import { Icon as IconV2 } from "@xpengagent/ui/v2/icon"
import { IconButtonV2 } from "@xpengagent/ui/v2/icon-button-v2"
import { MenuV2 } from "@xpengagent/ui/v2/menu-v2"
...
import { wslXpengagentAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.xpengagentChecks[item.config.distro]
          const xpengagentAction = () => wslXpengagentAction(check())
          const busy = () => wsl.data?.job?.kind === "install-xpengagent" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={xpengagentAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installXpengagent(item.config.distro))}
                    >
