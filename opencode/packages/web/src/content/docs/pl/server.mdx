---
title: Serwer
description: Interakcja z serwerem xpengagent za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `xpengagent serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient xpengagent.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `xpengagent.local` |
...
Ustaw `XPENGAGENT_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `xpengagent` lub ustaw `XPENGAGENT_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `xpengagent serve`, jak i `xpengagent web`.
...
Kiedy uruchomisz `xpengagent`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera xpengagent do programowej interakcji z xpengagent.
:::

Ta architektura umożliwia obsługę wielu klientów przez xpengagent i programową interakcję z kodem otwartym.

Możesz uruchomić `xpengagent serve`, aby uruchomić samodzielny serwer. Jeśli masz
xpengagent TUI działa, `xpengagent serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki xpengagent [IDE](/docs/ide).
...
Serwer xpengagent udostępnia następujące interfejsy API.
