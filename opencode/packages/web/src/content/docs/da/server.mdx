---
title: Server
description: Interager med xpengagent-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `xpengagent serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en xpengagent-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `xpengagent.local` |
...
Indstil `XPENGAGENT_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `xpengagent`, eller indstil `XPENGAGENT_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `xpengagent serve` og `xpengagent web`.
...
Når du kører `xpengagent` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug xpengagent-serveren til at interagere med xpengagent programmatisk.
:::

Denne arkitektur lader xpengagent understøtte flere klienter og lader dig interagere med xpengagent programmatisk.

Du kan køre `xpengagent serve` for at starte en selvstændig server. Hvis du har
xpengagent TUI kørende, vil `xpengagent serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af XPENGagent [IDE](/docs/ide) plugins.
...
XPENGagent-serveren viser følgende API'er.
