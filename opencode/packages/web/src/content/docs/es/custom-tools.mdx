...
description: Cree herramientas que LLM pueda llamar en xpengagent.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de xpengagent como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.xpengagent/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/xpengagent/tools/`.
...
```ts title=".xpengagent/tools/database.ts" {1}
...
```ts title=".xpengagent/tools/math.ts"
import { tool } from "@xpengagent/plugin"
...
```ts title=".xpengagent/tools/bash.ts"
import { tool } from "@xpengagent/plugin"
...
```ts title=".xpengagent/tools/project.ts" {8}
import { tool } from "@xpengagent/plugin"
...
```python title=".xpengagent/tools/add.py"
import sys
...
```ts title=".xpengagent/tools/python-add.ts" {10}
import { tool } from "@xpengagent/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".xpengagent/tools/add.py")
