---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć xpengagent.
---

Wtyczki umożliwiające wyłączenie xpengagent poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania xpengagent.
...
- `.xpengagent/plugins/` - Wtyczki na poziomie projektu
- `~/.config/xpengagent/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/xpengagent/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/xpengagent/xpengagent.json`)
2. Konfiguracja projektu (`xpengagent.json`)
3. Globalny katalog wtyczek (`~/.config/xpengagent/plugins/`)
4. Katalog wtyczek projektu (`.xpengagent/plugins/`)
...
xpengagent uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z xpengagent do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do xpengagent.
...
:::note
Jeśli korzystasz z aplikacji komputerowej xpengagent, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj xpengagent czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do xpengagent:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod xpengagent. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://xpengagent.cc.cd/docs/sdk).
