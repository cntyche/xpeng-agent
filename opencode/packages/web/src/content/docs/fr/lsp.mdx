---
title: Serveurs LSP
description: XPENGagent s'intègre à vos serveurs LSP.
---

XPENGagent peut s'intégrer aux serveurs Language Server Protocol (LSP) pour utiliser les diagnostics comme retour pour l'agent.

---

## Serveurs intégrés

XPENGagent est livré avec plusieurs serveurs LSP intégrés pour les langages populaires :
...
:::note
Vous pouvez désactiver les téléchargements automatiques du serveur LSP en définissant la variable d'environnement `XPENGAGENT_DISABLE_LSP_DOWNLOAD` sur `true`.
:::
...
Lorsque LSP est activé et qu'xpengagent ouvre un fichier, il :
...
Vous pouvez activer et personnaliser les serveurs LSP via la section `lsp` de votre configuration xpengagent.
...
```json title="xpengagent.json" {5-7}
{
  "$schema": "https://xpengagent.cc.cd/config.json",
  "lsp": {
    "rust": {
...
```json title="xpengagent.json" {5-9}
{
  "$schema": "https://xpengagent.cc.cd/config.json",
  "lsp": {
    "typescript": {
