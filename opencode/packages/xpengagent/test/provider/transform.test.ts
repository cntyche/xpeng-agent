    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".xpengagent/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const xpengagentModel = {
      ...openaiModel,
      providerID: "xpengagent",
      api: {
        id: "xpengagent-test",
        url: "https://api.xpengagent.cc.cd",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, xpengagentModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.xpengagent?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.xpengagent?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          xpengagent: { itemId: "msg_xpengagent" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              xpengagent: { itemId: "msg_xpengagent_part" },
    ...
    expect(result[0].providerOptions?.xpengagent?.itemId).toBe("msg_xpengagent")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.xpengagent?.itemId).toBe("msg_xpengagent_part")
