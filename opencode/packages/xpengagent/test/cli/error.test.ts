...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/xpengagent.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/xpengagent.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/xpengagent.jsonc", dir: ".xpengagent", suggestion: "xpengagent" },
        expected:
          'Directory "xpengagent" in /tmp/xpengagent.jsonc is not valid. Rename the directory to "xpengagent" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/xpengagent.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/xpengagent.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.xpengagent.cc.cd/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.xpengagent.cc.cd/auth/device/code.")
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `xpengagent models` to list available models",
      "Or check your config (xpengagent.json) provider/model names",
    ].join("\n")
