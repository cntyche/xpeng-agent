---
title: Servidor
description: Interactuar con el servidor xpengagent a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `xpengagent serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente xpengagent puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `xpengagent.local` |
...
Configure `XPENGAGENT_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `xpengagent`, o configure `XPENGAGENT_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `xpengagent serve` como a `xpengagent web`.
...
Cuando ejecuta `xpengagent`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor xpengagent para interactuar con xpengagent mediante programación.
:::

Esta arquitectura permite que xpengagent admita múltiples clientes y le permite interactuar con xpengagent mediante programación.

Puede ejecutar `xpengagent serve` para iniciar un servidor independiente. Si tienes el
xpengagent TUI ejecutándose, `xpengagent serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos XPENGagent [IDE](/docs/ide).
...
El servidor xpengagent expone las siguientes API.
