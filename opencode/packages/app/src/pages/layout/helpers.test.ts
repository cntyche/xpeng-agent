...
import { type Session } from "@xpengagent/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("xpengagent://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("xpengagent://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("xpengagent://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("xpengagent://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("xpengagent://open-project")).toBeUndefined()
    expect(parseDeepLink("xpengagent://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "xpengagent://open-project?directory=/a",
      "xpengagent://other?directory=/b",
      "xpengagent://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("xpengagent://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("xpengagent://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("xpengagent://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("xpengagent://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "xpengagent://new-session?directory=/a",
      "xpengagent://open-project?directory=/b",
      "xpengagent://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __XPENGAGENT__: {
        deepLinks: ["xpengagent://open-project?directory=/a"],
      },
    } as unknown as Window & { __XPENGAGENT__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["xpengagent://open-project?directory=/a"])
