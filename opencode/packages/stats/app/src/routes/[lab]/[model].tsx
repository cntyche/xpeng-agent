...
import { ProviderIcon } from "@xpengagent/ui/provider-icon"
...
import {
  ...
  type StatsModelData,
  type UsageRange,
} from "@xpengagent/stats-core/domain/home"
import { runtime } from "@xpengagent/stats-core/runtime"
...
const statsCanonicalBaseUrl = "https://xpengagent.cc.cd/data/"
const statsUnfurlPath = "banner.png"
const statsUnfurlAlt = "XPENGagent Data wordmark on a dark patterned background"
...
export default function StatsModel() {
  ...
  const modelTitle = createMemo(() => `${modelName()} Usage, Cost & Rank | XPENGagent Data`)
  const modelDescription = createMemo(
    () =>
      `View ${modelName()} XPENGagent Go usage data, including token volume, weekly rank, token mix, costs, cache ratio, sessions, geo breakdowns, and peer models.`,
  )
  ...
  return (
    <main data-page="stats" data-theme={themePreference()}>
      ...
      <Meta property="og:site_name" content="XPENGagent" />
      ...
      <div data-slot="model-hero-grid">
        <div data-slot="model-hero-copy">
          ...
          <Show
            when={props.data}
            fallback={
              <p>Model facts from the shared model index. XPENGagent Go usage appears once this model has activity.</p>
            }
          >
            {(data) => (
              <p>
                {data().rank === null
                  ? "Unranked across last week's XPENGagent Go usage"
                  : `Ranked #${data().rank} across last week's XPENGagent Go usage`}{" "}
...
function ModelCatalogCallout(props: { catalog: ModelCatalogEntry | null }) {
  return (
    <div data-component="model-rank-panel">
      ...
      <p>No XPENGagent Go usage in the current data window.</p>
    </div>
  )
}
...
function ModelOverview(props: { data: StatsModelData | null }) {
  return (
    <section data-section="model-panel">
      <SectionTitle title="Overview" description="Recent XPENGagent Go tokens, sessions, and market position." />
      <Show
        when={props.data}
        fallback={
          <ModelEmptyState title="No usage summary" description="This model has no XPENGagent Go usage rows yet." />
        }
      >
        {(data) => (
...
function ModelUsageSection(props: { data: ModelUsagePoint[] }) {
  ...
  return (
    <section id="usage" data-section="model-panel">
      <SectionTitle title="Usage" description="Daily XPENGagent Go token volume over the recent two-month window." />
      <Show
...
function ModelEfficiencySection(props: { data: StatsModelData | null; catalog: ModelCatalogEntry | null }) {
  return (
    <section id="efficiency" data-section="model-panel">
      <SectionTitle title="Efficiency" description="Cost, cache behavior, and average XPENGagent Go session shape." />
      <Show
        when={props.data}
        fallback={
...
function ModelGeoBreakdownSection(props: { data: Record<UsageRange, CountryEntry[]> }) {
  ...
  return (
    ...
    >
      <SectionTitle title="Geo Breakdown" description="XPENGagent Go model tokens used by country." />
      <Show
        when={data().length > 0}
        fallback={
          <ModelEmptyState title="No geo data" description="No XPENGagent Go geo_stat rows matched this model." />
        }
      >
...
function ModelPeersSection(props: { data: StatsModelData | null }) {
  return (
    <section id="peers" data-section="model-panel">
      <SectionTitle title="Peers" description="Nearby models by recent XPENGagent Go token volume." />
      <Show
