...
أنشئ الملف `.xpengagent/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات XPENGagent أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ XPENGagent:
...
- على مستوى النظام: `~/.config/xpengagent/commands/`
- لكل مشروع: `.xpengagent/commands/`
...
يتضمن xpengagent عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
