...
import { AbsolutePath, Location, Model, XPENGagent, Session, Tool } from "@xpengagent/core/public"
import { tmpdir } from "./fixture/tmpdir"
import { testEffect } from "./lib/effect"

const it = testEffect(XPENGagent.layer)

describe("public native XPENGagent API", () => {
  it.effect("exposes only the intentional Session capabilities", () =>
    Effect.gen(function* () {
      const xpengagent = yield* XPENGagent.Service

      expect(Object.keys(xpengagent).sort()).toEqual(["sessions", "tools"])

      expect(Object.keys(xpengagent.sessions).sort()).toEqual([
        "context",
        "create",
        "events",
        "get",
      ...
      expect(yield* xpengagent.sessions.list()).toBeArray()
      yield* xpengagent.tools.register({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          yield* xpengagent.sessions.create({
            id: sessionID,
            location: Location.Ref.make({ directory: AbsolutePath.make(tmp.path) }),
          })

          yield* xpengagent.sessions.switchModel({ sessionID, model })

          expect((yield* xpengagent.sessions.get(sessionID)).model).toEqual(model)
        }),
      ),
    ),
  )
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap(([available, disabled]) =>
        Effect.gen(function* () {
          ...
          yield* xpengagent.sessions.switchModel({ sessionID: availableID, model: ref({ variant: "default" }) })
          const disabledError = yield* xpengagent.sessions
            .switchModel({ sessionID: disabledID, model: ref() })
            .pipe(Effect.flip)
          const missingError = yield* xpengagent.sessions
          ...
          expect((yield* xpengagent.sessions.get(availableID)).model).toEqual(ref({ variant: "default" }))
          expect((yield* xpengagent.sessions.get(disabledID)).model).toBeUndefined()
        }),
      ),
    ),
  )
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          yield* xpengagent.sessions.switchModel({ sessionID, model: selected })

          const error = yield* xpengagent.sessions
            .switchModel({ sessionID, model: ref({ variant: "unknown" }) })
            .pipe(Effect.flip)

          expect(error).toBeInstanceOf(Session.VariantUnavailableError)
          expect((yield* xpengagent.sessions.get(sessionID)).model).toEqual(selected)
        }),
      ),
    ),
  )
