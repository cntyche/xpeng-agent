...
As habilidades do agente permitem que o xpengagent descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O xpengagent pesquisa nesses locais:

- Configuração do projeto: `.xpengagent/skills/<name>/SKILL.md`
- Configuração global: `~/.config/xpengagent/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o xpengagent sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.xpengagent/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/xpengagent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.xpengagent/skills/git-release/SKILL.md` assim:
...
O xpengagent lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `xpengagent.json`:
...
**Para agentes embutidos** (em `xpengagent.json`):
