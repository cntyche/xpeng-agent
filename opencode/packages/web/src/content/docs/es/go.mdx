...
XPENGagent Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

Go funciona como cualquier otro proveedor en XPENGagent. Te suscribes a XPENGagent Go y
...
XPENGagent Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

XPENGagent Go funciona como cualquier otro proveedor en XPENGagent.

1. Inicias sesión en **<a href={console}>XPENGagent Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `XPENGagent Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a XPENGagent Go.
:::
...
XPENGagent Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de XPENGagent
usa el formato `xpengagent-go/<model-id>`. Por ejemplo, para Kimi K2.7 Code, usarías
`xpengagent-go/kimi-k2.7-code` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con XPENGagent.
