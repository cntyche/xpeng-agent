...
Le skill degli agenti permettono a XPENGagent di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
XPENGagent cerca in queste posizioni:

- Config di progetto: `.xpengagent/skills/<name>/SKILL.md`
- Config globale: `~/.config/xpengagent/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, XPENGagent risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.xpengagent/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/xpengagent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.xpengagent/skills/git-release/SKILL.md` cosi':
...
XPENGagent elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `xpengagent.json`:
...
**Per agenti integrati** (in `xpengagent.json`):
