...
Las habilidades del agente permiten a XPENGagent descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
XPENGagent busca estas ubicaciones:

- Configuración del proyecto: `.xpengagent/skills/<name>/SKILL.md`
- Configuración global: `~/.config/xpengagent/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, XPENGagent sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.xpengagent/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/xpengagent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.xpengagent/skills/git-release/SKILL.md` así:
...
XPENGagent enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `xpengagent.json`:
...
**Para agentes integrados** (en `xpengagent.json`):
