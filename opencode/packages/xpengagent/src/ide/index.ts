...
export function alreadyInstalled() {
  return process.env["XPENGAGENT_CALLER"] === "vscode" || process.env["XPENGAGENT_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.xpengagent"], {
    nothrow: true,
  })
