...
const original = {
  XPENGAGENT_SERVER_PASSWORD: Flag.XPENGAGENT_SERVER_PASSWORD,
  XPENGAGENT_SERVER_USERNAME: Flag.XPENGAGENT_SERVER_USERNAME,
  envPassword: process.env.XPENGAGENT_SERVER_PASSWORD,
  envUsername: process.env.XPENGAGENT_SERVER_USERNAME,
}
const auth = { username: "xpengagent", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.XPENGAGENT_SERVER_PASSWORD = original.XPENGAGENT_SERVER_PASSWORD
  Flag.XPENGAGENT_SERVER_USERNAME = original.XPENGAGENT_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.XPENGAGENT_SERVER_PASSWORD
  else process.env.XPENGAGENT_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.XPENGAGENT_SERVER_USERNAME
  else process.env.XPENGAGENT_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.XPENGAGENT_SERVER_PASSWORD = auth.password
  Flag.XPENGAGENT_SERVER_USERNAME = auth.username
  process.env.XPENGAGENT_SERVER_PASSWORD = auth.password
  process.env.XPENGAGENT_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.XPENGAGENT_SERVER_PASSWORD = undefined
  Flag.XPENGAGENT_SERVER_USERNAME = auth.username
  delete process.env.XPENGAGENT_SERVER_PASSWORD
  process.env.XPENGAGENT_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-xpengagent-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-xpengagent-ticket": "1" }),
...
describe("HttpApi Server.listen", () => {
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-xpengagent-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.XPENGAGENT_DISABLE_DEFAULT_PLUGINS
    process.env.XPENGAGENT_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.XPENGAGENT_DISABLE_DEFAULT_PLUGINS
      else process.env.XPENGAGENT_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-xpengagent-ticket": "1" },
      })
