...
Com o xpengagent, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o xpengagent usa nosso próprio tema `xpengagent`.
...
O xpengagent vem com vários temas embutidos.
...
- Querem que o xpengagent corresponda à aparência do seu terminal
...
O xpengagent suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/xpengagent/themes/*.json` ou `$XDG_CONFIG_HOME/xpengagent/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.xpengagent/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.xpengagent/themes/*.json`
