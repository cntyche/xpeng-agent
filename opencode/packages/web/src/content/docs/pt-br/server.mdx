---
title: Servidor
description: Interaja com o servidor xpengagent via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `xpengagent serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente xpengagent pode usar.
...
```bash
xpengagent serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `xpengagent.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
xpengagent serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `XPENGAGENT_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `xpengagent`, ou defina `XPENGAGENT_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `xpengagent serve` quanto ao `xpengagent web`.

```bash
XPENGAGENT_SERVER_PASSWORD=your-password xpengagent serve
```
...
Quando você executa `xpengagent`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor xpengagent para interagir com o xpengagent programaticamente.
:::

Essa arquitetura permite que o xpengagent suporte múltiplos clientes e permite que você interaja com o xpengagent programaticamente.

Você pode executar `xpengagent serve` para iniciar um servidor autônomo. Se você tiver o TUI do xpengagent em execução, `xpengagent serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do xpengagent [IDE](/docs/ide).
...
O servidor xpengagent expõe as seguintes APIs.
