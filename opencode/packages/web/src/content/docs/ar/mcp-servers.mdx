...
يمكنك إضافة أدوات خارجية إلى XPENGagent باستخدام _Model Context Protocol_، أو MCP. يدعم XPENGagent خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات XPENGagent](https://xpengagent.cc.cd/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/xpengagent` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى XPENGagent تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم XPENGagent بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك XPENGagent المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `xpengagent mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن XPENGagent الرموز بشكل آمن في `~/.local/share/xpengagent/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل XPENGagent إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات XPENGagent مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط XPENGagent بحساب Sentry الخاص بك.
