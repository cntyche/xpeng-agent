...
Umiejętności agenta pozwalają xpengagent odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
xpengagent przeszukuje te lokalizacje:

- Project config: `.xpengagent/skills/<name>/SKILL.md`
- Global config: `~/.config/xpengagent/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu xpengagent przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.xpengagent/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/xpengagent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.xpengagent/skills/git-release/SKILL.md` w ten sposób:
...
xpengagent wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `xpengagent.json`:
...
**Dla agentów wbudowanych** (w `xpengagent.json`):
