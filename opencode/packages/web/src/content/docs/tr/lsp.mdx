...
description: XPENGagent, LSP sunucularınızla bütünleşir.
---

XPENGagent, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

XPENGagent, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`XPENGAGENT_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve xpengagent bir dosyayı açtığında:
...
LSP sunucularını, xpengagent yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
