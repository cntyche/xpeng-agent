---
title: Server
description: Nutze den xpengagent-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `xpengagent serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein xpengagent-Client nutzen kann.
...
Setze `XPENGAGENT_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `xpengagent`, kann aber mit `XPENGAGENT_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `xpengagent serve` und `xpengagent web`.
...
Wenn du `xpengagent` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den xpengagent-Server, um xpengagent programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `xpengagent serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `xpengagent serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die XPENGagent-[IDE](/docs/ide)-Plugins.
...
Der xpengagent-Server stellt folgende APIs bereit.
