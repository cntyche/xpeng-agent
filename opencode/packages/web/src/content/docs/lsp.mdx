---
title: LSP Servers
description: XPENGagent integrates with your LSP servers.
---

XPENGagent can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

XPENGagent comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `XPENGAGENT_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and xpengagent opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your xpengagent config.
...
```json title="xpengagent.json" {5-8}
{
  "$schema": "https://xpengagent.cc.cd/config.json",
  "lsp": {
    "rust": {
...
```json title="xpengagent.json" {5-13}
{
  "$schema": "https://xpengagent.cc.cd/config.json",
  "lsp": {
    "custom-lsp": {
