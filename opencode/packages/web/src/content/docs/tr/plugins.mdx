---
title: Eklentiler
description: XPENGagent'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek XPENGagent'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya XPENGagent'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.xpengagent/plugins/` - Proje düzeyinde eklentiler
- `~/.config/xpengagent/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/xpengagent/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/xpengagent/xpengagent.json`)
2. Project config (`xpengagent.json`)
3. Global eklenti dizini (`~/.config/xpengagent/plugins/`)
4. Proje eklenti dizini (`.xpengagent/plugins/`)
...
XPENGagent bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir XPENGagent SDK istemcisi.
...
XPENGagent'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
XPENGagent masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
XPENGagent'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca XPENGagent'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, XPENGagent'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra XPENGagent'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://xpengagent.cc.cd/docs/sdk) bakın.
