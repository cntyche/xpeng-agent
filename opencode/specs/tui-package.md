...
`packages/xpengagent/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/xpengagent`,
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/xpengagent`.
- Do not replace private `packages/xpengagent` imports with `@opencode-ai/core`
...
```text
packages/xpengagent/src/cli/cmd/tui
```

Its private dependency on `packages/xpengagent` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/xpengagent/src/*`.
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/xpengagent` initially.
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/xpengagent`.
...
- The package has no imports from `packages/xpengagent`, `packages/cli`, or
  `@opencode-ai/core`.
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/xpengagent`.
- Keep the legacy embedded worker and server startup in `packages/xpengagent`.
...
- Remove obsolete `@tui/*` path mappings from `packages/xpengagent`.
...
cd packages/xpengagent && bun typecheck
...
rg '@opencode-ai/core|packages/xpengagent|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/xpengagent/src packages/xpengagent/test
```
...
- Build the current-platform `packages/xpengagent` binary.
