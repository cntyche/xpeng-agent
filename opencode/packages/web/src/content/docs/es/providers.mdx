---
title: Proveedores
description: Usando cualquier proveedor LLM en XPENGagent.
---

import config from "../../../../config.mjs"
export const console = config.console

XPENGagent usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración XPENGagent.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu XPENGagent
configuración.
...
XPENGagent Zen es una lista de modelos proporcionados por el equipo XPENGagent que han sido
probado y verificado para funcionar bien con XPENGagent. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con XPENGagent Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione xpengagent y diríjase a [xpengagent.cc.cd/auth](https://xpengagent.cc.cd/auth).
...
Funciona como cualquier otro proveedor en XPENGagent y su uso es completamente opcional.

---

## XPENGagent Go

XPENGagent Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de XPENGagent que han sido
probado y verificado para funcionar bien con XPENGagent.

1. Ejecute el comando `/connect` en TUI, seleccione `XPENGagent Go` y diríjase a [xpengagent.cc.cd/auth](https://xpengagent.cc.cd/zen).
...
Para usar Amazon Bedrock con XPENGagent:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta xpengagent:
   ...
   Para una configuración persistente o específica del proyecto, utilice `xpengagent.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en XPENGagent no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar xpengagent para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que xpengagent funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración xpengagent.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `XPENGagent`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación xpengagent.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
XPENGagent utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear XPENGagent
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `xpengagent.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [xpengagent-gitlab-auth](https://www.npmjs.com/package/xpengagent-gitlab-auth).

##### Configuración

Personalizar a través de `xpengagent.json`:
...
Para utilizar su suscripción GitHub Copilot con xpengagent:
...
Para utilizar Google Vertex AI con XPENGagent:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta xpengagent.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de xpengagent, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [xpengagent-helicone-session](https://github.com/H2Shami/xpengagent-helicone-session) para registrar automáticamente cada conversación XPENGagent como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación XPENGagent enumerada como una sesión separada.
...
Puede configurar xpengagent para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar xpengagent para usar modelos locales a través de LM Studio.
...
Puedes configurar xpengagent para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para XPENGagent. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/xpengagent) para obtener más detalles.
:::
...
Para usar Ollama Cloud con XPENGagent:
...
3. Copie la clave API para usarla en XPENGagent.
...
6. **Importante**: Antes de usar modelos en la nube en XPENGagent, debe obtener la información del modelo localmente:
...
XPENGagent Zen es una lista de modelos probados y verificados proporcionada por el equipo XPENGagent. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>XPENGagent Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **XPENGagent Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave XPENGagent API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración xpengagent.
...
5. También puedes personalizarlos a través de tu configuración xpengagent. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con XPENGagent:
...
También puede personalizar modelos a través de su configuración xpengagent. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con xpengagent. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `xpengagent.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a XPENGagent comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `xpengagent auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración xpengagent y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración xpengagent.
