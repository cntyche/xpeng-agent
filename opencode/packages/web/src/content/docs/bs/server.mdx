---
title: Server
description: Komunicirajte s xpengagent serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `xpengagent serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji xpengagent klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `xpengagent.local` |
...
Postavite `XPENGAGENT_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `xpengagent`, ili postavite `XPENGAGENT_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `xpengagent serve` i za `xpengagent web`.
...
Kada pokrenete `xpengagent`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite xpengagent server za programsku interakciju sa xpengagent-om.
:::

Ova arhitektura omogucava xpengagent podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `xpengagent serve` da startate standalone server. Ako je xpengagent TUI vec pokrenut, `xpengagent serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste XPENGagent [IDE](/docs/ide) pluginovi.
...
xpengagent server izlaže sljedece API-je.
