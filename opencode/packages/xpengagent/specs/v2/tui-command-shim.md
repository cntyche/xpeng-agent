...
Delete `packages/xpengagent/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/xpengagent/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/xpengagent/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/xpengagent`
- TUI plugin loader tests in `packages/xpengagent` if runtime plugin API wiring changed
