---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i XPENGagent.
---

import config from "../../../../config.mjs"
export const console = config.console

XPENGagent bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i XPENGagent-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/xpengagent/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i XPENGagent-konfigurationen.
...
XPENGagent Zen er en liste over modeller leveret af XPENGagent-teamet, som er blevet testet og verificeret til at fungere godt med XPENGagent. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med XPENGagent Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg xpengagent og gå til [xpengagent.cc.cd/auth](https://xpengagent.cc.cd/auth).
...
Det fungerer som alle andre udbydere i XPENGagent og er helt valgfrit at bruge.

---

## XPENGagent Go

XPENGagent Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af XPENGagent-teamet, som er testet og verificeret til at fungere godt med XPENGagent.

1. Kør kommandoen `/connect` i TUI, vælg `XPENGagent Go`, og gå til [xpengagent.cc.cd/auth](https://xpengagent.cc.cd/zen).
...
Sådan bruger du Amazon Bedrock med XPENGagent:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører xpengagent:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `xpengagent.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i XPENGagent understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere xpengagent til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at xpengagent skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem xpengagent-konfigurationen.
...
gemme tokenet i xpengagent auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
XPENGagent bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse XPENGagent
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`xpengagent.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [xpengagent-gitlab-auth](https://www.npmjs.com/package/xpengagent-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `xpengagent.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med XPENGagent:
...
Sådan bruger du Google Vertex AI med XPENGagent:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører xpengagent.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem xpengagent, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [xpengagent-helicone-session](https://github.com/H2Shami/xpengagent-helicone-session) plugin for automatisk at logge hver XPENGagent-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver XPENGagent-samtale opført som en separat session.
...
Du kan konfigurere xpengagent til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere xpengagent til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere xpengagent til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for XPENGagent. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/xpengagent) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med XPENGagent:
...
3. Kopier API-nøglen til brug i XPENGagent.
...
6. **Vigtigt**: Før du bruger skymodeller i XPENGagent, skal du hente modelinformationen lokalt:
...
XPENGagent Zen er en liste over testede og verificerede modeller leveret af XPENGagent-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>XPENGagent Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **XPENGagent Zen**.

   ```txt
   /connect
   ```

3. Indtast XPENGagent API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem xpengagent-konfigurationen.
...
5. Du kan også tilpasse dem gennem xpengagent-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med XPENGagent:
...
Du kan også tilpasse modeller gennem xpengagent-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med xpengagent. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `xpengagent.json`-filen i projektkataloget:
...
`limit`-felterne lader XPENGagent forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `xpengagent auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek xpengagent-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i xpengagent-konfigurationen.
