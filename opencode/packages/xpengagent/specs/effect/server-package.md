...
- The server still lives in `packages/xpengagent`.
...
Until enough shared service code lives outside `packages/xpengagent`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/xpengagent`

It should not import `packages/xpengagent` services while `packages/xpengagent`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/xpengagent` implementation details.
