import { Button } from "@xpengagent/ui/button"
import { useDialog } from "@xpengagent/ui/context/dialog"
import { Spinner } from "@xpengagent/ui/spinner"
import { showToast } from "@xpengagent/ui/toast"
import { createEffect, createMemo, For, Match, onCleanup, Show, Switch } from "solid-js"
import { createStore } from "solid-js/store"
import { useLanguage } from "@/context/language"
import { usePlatform } from "@/context/platform"
import { useWslServers } from "./context"
import { enterWslXpengagentStep } from "./settings-model"

type WslServerStep = "wsl" | "distro" | "xpengagent"

const STEPS: WslServerStep[] = ["wsl", "distro", "xpengagent"]
...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const xpengagentCheck = createMemo(() => {
    const distro = selectedDistro()
    if (!distro) return null
    return current()?.xpengagentChecks[distro] ?? null
  })
  ...
  const xpengagentReady = createMemo(() => {
    const check = xpengagentCheck()
  ...
  const installingXpengagent = createMemo(() => {
    const job = current()?.job
    return job?.kind === "install-xpengagent" && job.distro === selectedDistro()
  })
  const allReady = createMemo(() => wslReady() && distroReady() && xpengagentReady())
  const addDisabled = createMemo(() => {
    const job = current()?.job
    if (!job) return store.adding
    return store.adding || job.kind !== "probe-xpengagent"
  })
  ...
  const autoProbe = createMemo(() => {
    ...
    if (!state.xpengagentChecks[distro]) {
      return { key: `probe-xpengagent:${distro}`, run: () => api.probeXpengagent(distro) }
    }
    return null
  })
  ...
  const xpengagentMessage = createMemo(() => {
    const state = current()
    if (!state) return language.t("wsl.onboarding.checkingXpengagent")
    const distro = selectedDistro()
    if (state.job?.kind === "install-xpengagent") {
      return distro
        ? language.t("wsl.onboarding.updatingXpengagentIn", { distro })
        : language.t("wsl.onboarding.updatingXpengagent")
    }
    if (state.job?.kind === "probe-xpengagent") {
      return distro
        ? language.t("wsl.onboarding.checkingXpengagentIn", { distro })
        : language.t("wsl.onboarding.checkingXpengagent")
    }
    if (xpengagentCheck()?.error) return xpengagentCheck()!.error
    if (xpengagentCheck()?.matchesDesktop === false) {
      return distro
        ? language.t("wsl.onboarding.updateXpengagentIn", { distro })
        : language.t("wsl.onboarding.updateXpengagent")
    }
    if (xpengagentReady()) {
      return distro
        ? language.t("wsl.onboarding.xpengagentReadyIn", { distro })
        : language.t("wsl.onboarding.xpengagentReady")
    }
    return distro
      ? language.t("wsl.onboarding.installXpengagentIn", { distro })
  ...
  const openXpengagentStep = () => {
    const distro = selectedDistro()
    if (!distro) return
    void run(() => enterWslXpengagentStep(distro, api.probeXpengagent, (step) => setStore("step", step)))
  }
        ...
        title:
          step === "wsl"
            ...
            : step === "distro"
              ...
              : language.t("wsl.onboarding.step.xpengagent"),
        state:
          active === step
            ? "current"
            : step === "wsl"
              ...
              : step === "distro"
                ? distroReady()
                  ...
                  : index > activeIndex
                    ? "locked"
                    : "warning"
                : xpengagentCheck()?.matchesDesktop === false
                  ? "warning"
                  : xpengagentReady()
                    ? "done"
                    : index > activeIndex
      ...
      >
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "xpengagent"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.xpengagent")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.probeXpengagent(distro))}
                      >
                        {language.t("wsl.onboarding.refresh")}
                      </Button>
                    </Show>
                    <Show when={!xpengagentReady() || xpengagentCheck()?.matchesDesktop === false}>
                      <Button
                        variant="secondary"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.installXpengagent(distro))}
                      >
                        <Show when={installingXpengagent()}>
                          <Spinner class="size-4 shrink-0" />
                        </Show>
                        {xpengagentCheck()?.resolvedPath
                          ? language.t("wsl.onboarding.updateXpengagent")
                          : language.t("wsl.onboarding.installXpengagent")}
                      </Button>
                    </Show>
                  </div>
                </div>
                <div class="text-12-regular text-text-weak whitespace-pre-wrap break-words">{xpengagentMessage()}</div>
                <Show when={xpengagentCheck()?.matchesDesktop === false ? xpengagentCheck() : null}>
                  {(check) => (
                    <div class="rounded-md border border-border-weak-base px-3 py-3 flex flex-col gap-1">
                      <div class="text-12-regular text-text-weak">
          ...
          <Show when={activeStep() === "xpengagent" && allReady() && selectedDistro()}>
