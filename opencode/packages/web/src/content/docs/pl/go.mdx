...
XPENGagent Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w XPENGagent. Subskrybujesz XPENGagent Go i
...
XPENGagent Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.

---

## Jak to działa

XPENGagent Go działa jak każdy inny dostawca w XPENGagent.

1. Logujesz się do **<a href={console}>XPENGagent Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `XPENGagent Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować XPENGagent Go.
:::
...
XPENGagent Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji XPENGagent
używa formatu `xpengagent-go/<model-id>`. Na przykład dla Kimi K2.7 Code należy użyć
`xpengagent-go/kimi-k2.7-code` w swojej konfiguracji.
...
Stworzyliśmy XPENGagent Go, aby:
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z XPENGagent.
