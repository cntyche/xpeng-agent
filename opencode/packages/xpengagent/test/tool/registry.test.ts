...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".xpengagent")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .xpengagent/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const xpengagent = path.join(test.directory, ".xpengagent")
      const tool = path.join(xpengagent, "tool")
  ...
  it.instance("ignores non-tool exports in .xpengagent/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".xpengagent", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-xpengagent` was registering a tool
  ...
  it.instance("loads tools from .xpengagent/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const xpengagent = path.join(test.directory, ".xpengagent")
      const tools = path.join(xpengagent, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".xpengagent", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const xpengagent = path.join(test.directory, ".xpengagent")
        const customTools = path.join(xpengagent, "tools")
        const plugin = path.join(xpengagent, "node_modules", "@xpengagent", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(xpengagent, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@xpengagent/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@xpengagent/plugin"',
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".xpengagent", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(xpengagent, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@xpengagent/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(xpengagent, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(xpengagent, "node_modules", "cowsay")
