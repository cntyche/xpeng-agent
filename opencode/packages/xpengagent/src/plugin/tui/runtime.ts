...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@xpengagent/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { errorData, errorMessage } from "@xpengagent/tui/util/error"
...
import { hasTheme, upsertTheme } from "@xpengagent/tui/context/theme"
import { Global } from "@xpengagent/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@xpengagent/core/util/flock"
import { Flag } from "@xpengagent/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@xpengagent/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@xpengagent/core/v1/config/plugin"
import { createCommandShim } from "@xpengagent/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@xpengagent/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".xpengagent"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".xpengagent", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".xpengagent", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.XPENGAGENT_PURE ? [] : pluginOrigins
    if (Flag.XPENGAGENT_PURE && pluginOrigins.length) {
    }
