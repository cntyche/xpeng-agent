...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-xpengagent-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-xpengagent-directory"] ? decode(request.headers["x-xpengagent-directory"]) : process.cwd())
