...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall xpengagent and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall XPENGagent")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g xpengagent",
      pnpm: "pnpm uninstall -g xpengagent",
      bun: "bun remove -g xpengagent",
      yarn: "yarn global remove xpengagent",
      brew: "brew uninstall xpengagent",
      choco: "choco uninstall xpengagent",
      scoop: "scoop uninstall xpengagent",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "xpengagent", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using XPENGagent!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# xpengagent") || content.includes(".xpengagent/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".xpengagent/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".xpengagent/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".xpengagent"))
    ) {
      continue
    }
