---
title: Complementos
description: Escriba sus propios complementos para extender XPENGagent.
---

Los complementos le permiten extender XPENGagent al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de XPENGagent.
...
- `.xpengagent/plugins/` - Complementos a nivel de proyecto
- `~/.config/xpengagent/plugins/` - Complementos globales
...
```json title="xpengagent.json"
{
  ...
  "plugin": ["xpengagent-helicone-session", "xpengagent-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/xpengagent/node_modules/`.
...
1. Configuración global (`~/.config/xpengagent/xpengagent.json`)
2. Configuración del proyecto (`xpengagent.json`)
3. Directorio global de complementos (`~/.config/xpengagent/plugins/`)
4. Directorio de complementos del proyecto (`.xpengagent/plugins/`)
...
```json title=".xpengagent/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
XPENGagent ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".xpengagent/plugins/my-plugin.ts"
...
```js title=".xpengagent/plugins/example.js"
...
- `client`: Un cliente SDK xpengagent para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar xpengagent.
...
```js title=".xpengagent/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "xpengagent"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio XPENGagent, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que xpengagent lea archivos `.env`:

```javascript title=".xpengagent/plugins/env-protection.js"
...
```javascript title=".xpengagent/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a xpengagent:

```ts title=".xpengagent/plugins/custom-tools.ts"
import { type Plugin, tool } from "@xpengagent/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que xpengagent puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para xpengagent junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://xpengagent.cc.cd/docs/sdk) para obtener más detalles.
...
```ts title=".xpengagent/plugins/compaction.ts"
import type { Plugin } from "@xpengagent/plugin"
...
```ts title=".xpengagent/plugins/custom-compaction.ts"
import type { Plugin } from "@xpengagent/plugin"
