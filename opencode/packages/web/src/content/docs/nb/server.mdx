---
title: Tjener
description: Samhandle med XPENGagent-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `xpengagent serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en XPENGagent-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `xpengagent.local` |
...
Sett `XPENGAGENT_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `xpengagent` som standard, eller sett `XPENGAGENT_SERVER_USERNAME` for å overstyre det. Dette gjelder både `xpengagent serve` og `xpengagent web`.
...
Når du kjører `xpengagent` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk XPENGagent-serveren til å samhandle med XPENGagent programmatisk.
:::

Denne arkitekturen lar XPENGagent støtte flere klienter og lar deg samhandle med XPENGagent programmatisk.

Du kan kjøre `xpengagent serve` for å starte en frittstående server. Hvis du har
XPENGagent TUI kjører, vil `xpengagent serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av XPENGagent [IDE](/docs/ide) plugins.
...
xpengagent-serveren viser følgende APIer.
