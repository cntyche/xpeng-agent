...
XPENGagent Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u XPENGagent-u. Pretplatite se na XPENGagent Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili XPENGagent.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa XPENGagent-om.
:::
...
XPENGagent Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

XPENGagent Go radi kao bilo koji drugi provajder u XPENGagent-u.

1. Prijavite se na **<a href={console}>XPENGagent Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `XPENGagent Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na XPENGagent Go.
:::
...
XPENGagent Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj XPENGagent konfiguraciji
koristi format `xpengagent-go/<model-id>`. Na primjer, za Kimi K2.7 Code, koristili biste
`xpengagent-go/kimi-k2.7-code` u svojoj konfiguraciji.
...
Napravili smo XPENGagent Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz XPENGagent koristite i bilo kojeg drugog provajdera.
