...
Vous pouvez ajouter des outils externes à XPENGagent à l'aide du _Model Context Protocol_ ou MCP. XPENGagent prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [XPENGagent Config](https://xpengagent.cc.cd/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/xpengagent`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
XPENGagent gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, XPENGagent :
...
Si le serveur nécessite une authentification, XPENGagent vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `xpengagent mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, XPENGagent stockera les jetons en toute sécurité dans `~/.local/share/xpengagent/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans XPENGagent, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration XPENGagent comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter XPENGagent à votre compte Sentry.
