---
title: Serveur
description: Interagissez avec le serveur xpengagent via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `xpengagent serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client xpengagent peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `xpengagent.local` |
...
Définissez `XPENGAGENT_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `xpengagent`, ou définissez `XPENGAGENT_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `xpengagent serve` et à `xpengagent web`.
...
Lorsque vous exécutez `xpengagent`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur xpengagent pour interagir avec xpengagent par programmation.
:::

Cette architecture permet à xpengagent de prendre en charge plusieurs clients et vous permet d'interagir avec xpengagent par programme.

Vous pouvez exécuter `xpengagent serve` pour démarrer un serveur autonome. Si vous avez le
xpengagent TUI en cours d'exécution, `xpengagent serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins XPENGagent [IDE](/docs/ide).
...
Le serveur xpengagent expose les API suivantes.
