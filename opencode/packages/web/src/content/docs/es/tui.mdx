---
title: TUI
description: Usando la interfaz de usuario del terminal XPENGagent.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

XPENGagent proporciona una interfaz de terminal interactiva o TUI para trabajar en sus proyectos con un LLM.

Al ejecutar XPENGagent se inicia TUI para el directorio actual.
...
Cuando utilice XPENGagent TUI, puede escribir `/` seguido de un nombre de comando para ejecutar acciones rápidamente. Por ejemplo:
...
Agregue un proveedor a XPENGagent. Le permite seleccionar entre proveedores disponibles y agregar sus claves API.
...
Salga de XPENGagent. _Alias_: `/quit`, `/q`
...
Esto es independiente de `xpengagent.json`, que configura el comportamiento del servidor/tiempo de ejecución.
...
- `leader_timeout`: controla cuánto tiempo espera XPENGagent después de la leader key. El valor predeterminado es `2000`.
...
- `sound_pack`: ID del sound pack que se usará. El valor predeterminado es `xpengagent.default`.
