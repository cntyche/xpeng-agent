---
title: Сервер
description: Взаимодействуйте с сервером xpengagent через HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Команда `xpengagent serve` запускает автономный HTTP-сервер, который предоставляет конечную точку OpenAPI, которую может использовать клиент с открытым кодом.
...
| `--mdns-domain` | Пользовательское доменное имя для mDNS      | `xpengagent.local` |
...
Установите `XPENGAGENT_SERVER_PASSWORD`, чтобы защитить сервер с помощью базовой аутентификации HTTP. Имя пользователя по умолчанию — `xpengagent` или установите `XPENGAGENT_SERVER_USERNAME`, чтобы переопределить его. Это относится как к `xpengagent serve`, так и к `xpengagent web`.
...
Когда вы запускаете `xpengagent`, он запускает TUI и сервер. Где находится TUI
...
:::tip
Используйте сервер xpengagent для программного взаимодействия с открытым кодом.
:::
...
Вы можете запустить `xpengagent serve`, чтобы запустить автономный сервер. Если у вас есть
TUI с открытым кодом запущен, `xpengagent serve` запустит новый сервер.
...
Конечную точку [`/tui`](#tui) можно использовать для управления TUI через сервер. Например, вы можете предварительно заполнить или запустить подсказку. Эта настройка используется плагинами xpengagent [IDE](/docs/ide).
...
Сервер xpengagent предоставляет следующие API.
