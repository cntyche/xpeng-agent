...
Možete dodati vanjske alate u XPENGagent koristeći _Model Context Protocol_, ili MCP. XPENGagent podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [XPENGagent Config](https://xpengagent.cc.cd/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/xpengagent`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
XPENGagent automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, XPENGagent će:
...
Ako server zahtijeva autentifikaciju, XPENGagent će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `xpengagent mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, XPENGagent sigurno čuva tokene u `~/.local/share/xpengagent/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u XPENGagent, zajedno s ugrađenim alatima. Možete njima upravljati kroz XPENGagent konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete XPENGagent sa vašim Sentry nalogom.
