...
describe("xpengagent read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // XPENGAGENT_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ xpengagent }) =>
      Effect.gen(function* () {
        const r = yield* xpengagent.spawn(["mcp", "list"])
        xpengagent.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ xpengagent }) =>
      Effect.gen(function* () {
        const r = yield* xpengagent.spawn(["providers", "list"])
        xpengagent.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ xpengagent }) =>
      Effect.gen(function* () {
        const r = yield* xpengagent.spawn(["models"])
        xpengagent.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ xpengagent }) =>
      Effect.gen(function* () {
        const r = yield* xpengagent.spawn(["agent", "list"])
        xpengagent.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh XPENGAGENT_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ xpengagent }) =>
      Effect.gen(function* () {
        const r = yield* xpengagent.spawn(["session", "list"])
        xpengagent.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ xpengagent }) =>
      Effect.gen(function* () {
        const r = yield* xpengagent.spawn(["stats"])
        xpengagent.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under XPENGAGENT_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ xpengagent }) =>
      Effect.gen(function* () {
        const r = yield* xpengagent.spawn(["db", "path"])
        xpengagent.expectExit(r, 0, "db path")
