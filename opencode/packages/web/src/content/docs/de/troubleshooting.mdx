...
Wenn XPENGagent Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\xpengagent\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `xpengagent --log-level DEBUG`.

---

## Speicher

xpengagent speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/xpengagent/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\xpengagent`
...
XPENGagent Desktop startet im Hintergrund einen lokalen XPENGagent-Server (`xpengagent-cli`-Sidecar).
...
- Nur macOS: `XPENGagent`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/xpengagent/xpengagent.jsonc` (oder `~/.config/xpengagent/xpengagent.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/xpengagent/xpengagent.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\xpengagent\xpengagent.jsonc`
...
XPENGagent kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/xpengagent/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\xpengagent\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit XPENGagent ihn neu aufbauen kann.

1. Beende XPENGagent Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/xpengagent`
- **Linux**: loesche `~/.cache/xpengagent` (oder fuehre aus: `rm -rf ~/.cache/xpengagent`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\xpengagent`

3. Starte XPENGagent Desktop neu.

---

### Server-Verbindungsprobleme beheben

XPENGagent Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `xpengagent.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `XPENGAGENT_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `XPENGAGENT_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt XPENGagent Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit XPENGagent-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

XPENGagent Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer XPENGagent in den OS-Einstellungen aktiviert sind, und
...
1. Beende XPENGagent Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von XPENGagent Desktop):

- `xpengagent.settings.dat` (desktop default server URL)
- `xpengagent.global.dat` und `xpengagent.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit XPENGagent hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `xpengagent models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\xpengagent`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. xpengagent installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\xpengagent`

2. Starte xpengagent neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt xpengagent, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
xpengagent erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
