...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. XPENGagent dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u XPENGagent.
...
:::note
Ovaj alat je dostupan samo kada je `XPENGAGENT_EXPERIMENTAL_LSP_TOOL=true` (ili `XPENGAGENT_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz XPENGagent provajdera ili kada je varijabla `XPENGAGENT_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju XPENGagent:
