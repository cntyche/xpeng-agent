...
Existem dois tipos de agentes no xpengagent; agentes primários e subagentes.
...
xpengagent vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
XPENGagent vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

XPENGagent vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do XPENGagent, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `xpengagent.json`:
...
- Global: `~/.config/xpengagent/agents/`
- Por projeto: `.xpengagent/agents/`

```markdown title="~/.config/xpengagent/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o xpengagent usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do xpengagent quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do xpengagent usa o formato `provider/model-id`. Por exemplo, se você estiver usando [XPENGagent Zen](/docs/zen), você usaria `xpengagent/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="xpengagent.json" {3-6,9-12}
{
  "$schema": "https://xpengagent.cc.cd/config.json",
  "tools": {
    "write": true,
...
```json title="xpengagent.json" {3-5,8-10}
{
  "$schema": "https://xpengagent.cc.cd/config.json",
  "permission": {
    "edit": "deny"
...
```json title="xpengagent.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Execute `xpengagent models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/xpengagent).
:::
...
```markdown title="~/.config/xpengagent/agents/docs-writer.md"
---
...
```markdown title="~/.config/xpengagent/agents/security-auditor.md"
---
