...
XPENGagent Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, XPENGagent'daki diğer sağlayıcılar gibi çalışır. XPENGagent Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve XPENGagent'u kullanmak için bunu kullanmanıza gerek yoktur.
...
XPENGagent Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

XPENGagent Go, XPENGagent'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>XPENGagent Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `XPENGagent Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye XPENGagent Go'ya abone olabilir.
:::
...
XPENGagent Go aşağıdaki limitleri içerir:
...
XPENGagent yapılandırmanızdaki [model id](/docs/config/#models) formatı `xpengagent-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `xpengagent-go/kimi-k2.7-code` kullanmalısınız.
...
XPENGagent Go'yu şu amaçlarla oluşturduk:
...
4. XPENGagent ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
