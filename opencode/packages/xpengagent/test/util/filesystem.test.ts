...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "xpengagent.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "xpengagent.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "xpengagent.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "xpengagent.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["xpengagent.json", "xpengagent.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "xpengagent.json"),
        path.join(project, "xpengagent.jsonc"),
      ])
    })
  })
