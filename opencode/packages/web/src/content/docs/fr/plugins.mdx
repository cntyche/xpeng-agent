---
title: Plugins
description: Écrivez vos propres plugins pour étendre XPENGagent.
---

Les plugins vous permettent d'étendre XPENGagent en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de XPENGagent.
...
- `.xpengagent/plugins/` - Plugins au niveau du projet
- `~/.config/xpengagent/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/xpengagent/node_modules/`.
...
1. Configuration globale (`~/.config/xpengagent/xpengagent.json`)
2. Configuration du projet (`xpengagent.json`)
3. Répertoire global des plugins (`~/.config/xpengagent/plugins/`)
4. Répertoire des plugins du projet (`.xpengagent/plugins/`)
...
XPENGagent exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK xpengagent pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre xpengagent.
...
:::note
Si vous utilisez l'application de bureau XPENGagent, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez xpengagent de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à xpengagent :
...
L'assistant `tool` crée un outil personnalisé que xpengagent peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour xpengagent aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://xpengagent.cc.cd/docs/sdk) pour plus de détails.
