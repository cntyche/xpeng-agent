...
import { LocationServiceMap } from "@xpengagent/core/location-layer"
...
it.instance("plan agent denies edits except .xpengagent/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".xpengagent/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".xpengagent", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.XPENGAGENT_TEST_HOME
      process.env.XPENGAGENT_TEST_HOME = test.directory
