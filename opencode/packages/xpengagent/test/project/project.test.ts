...
import { ProjectDirectories } from "@xpengagent/core/project/directories"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const xpengagentFile = path.join(tmp, ".git", "xpengagent")
      expect(yield* Effect.promise(() => Bun.file(xpengagentFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "xpengagent")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "xpengagent")
      const wrongCache = path.join(parentDir, ".git", "xpengagent")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "xpengagent")
      const cacheB = path.join(bareB, "xpengagent")
      const wrongCache = path.join(parentDir, ".git", "xpengagent")
