...
- Author package entrypoint is `@xpengagent/plugin/tui`.
...
- Import types from `@xpengagent/plugin/tui`.
- `@xpengagent/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@xpengagent/plugin/tui"
...
- The value is a semver range checked against the running XPENGagent version.
...
- `xpengagent plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `xpengagent plug <module>`.
...
When `plugin` entries exist in a writable `.opencode` dir or `XPENGAGENT_CONFIG_DIR`, XPENGagent installs `@xpengagent/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@xpengagent/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus XPENGagent's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
XPENGagent registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, XPENGagent pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@xpengagent/plugin/tui`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `xpengagent.default`.
...
- Local plugins persist installed themes under the local `.xpengagent/themes` area near the plugin config source.
...
- `--pure` / `XPENGAGENT_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.xpengagent/plugins/tui-smoke.tsx`
- Local vim plugin: `.xpengagent/plugins/tui-vim.tsx`
- Local smoke config: `.xpengagent/tui.json`
- Local smoke theme: `.xpengagent/plugins/smoke-theme.json`
