...
const TUI_SCHEMA_URL = "https://xpengagent.cc.cd/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from xpengagent.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const xpengagent = await xpengagentFiles(input)
  for (const file of xpengagent) {
...
async function xpengagentFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "xpengagent"),
    ...(await Filesystem.findUp(["xpengagent.json", "xpengagent.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "xpengagent"))
  }
  if (Flag.XPENGAGENT_CONFIG) files.push(Flag.XPENGAGENT_CONFIG)
