...
xpengagent ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak xpengagent kendi `xpengagent` temasını kullanır.
...
xpengagent birden fazla yerleşik temayla gelir.
...
- xpengagent'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
xpengagent, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/xpengagent/themes/*.json` veya `$XDG_CONFIG_HOME/xpengagent/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.xpengagent/themes/*.json`
4. **Geçerli çalışma dizini** - `./.xpengagent/themes/*.json`
