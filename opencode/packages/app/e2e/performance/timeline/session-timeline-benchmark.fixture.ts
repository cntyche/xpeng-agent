...
import { mockXPENGagentServer } from "../../utils/mock-server"
import { expectAppVisible, expectSessionTitle } from "../../utils/waits"
import { expect } from "../benchmark"

const directory = "C:/XPENGagent/TimelineStateRegression"
...
const model = { providerID: "xpengagent", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimelineBenchmark(page: Page, options: { historyTurns: number; eventBatch: number }) {
  ...
  await mockXPENGagentServer(page, {
    directory,
    project: project(),
    provider: provider(),
    sessions: [session()],
...
function provider() {
  return {
    ...
    connected: ["xpengagent"],
    default: { providerID: "xpengagent", modelID: "claude-opus-4-6" },
  }
}
