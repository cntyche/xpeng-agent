...
Du kan legge til eksterne verktøy i XPENGagent ved å bruke _Model Context Protocol_ eller MCP. XPENGagent støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [XPENGagent Config](https://xpengagent.cc.cd/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="xpengagent.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/xpengagent`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="xpengagent.jsonc" {15}
{
  "$schema": "https://xpengagent.cc.cd/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
XPENGagent håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil XPENGagent:
...
Hvis serveren krever autentisering, vil XPENGagent be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `xpengagent mcp auth <server-name>`.
...
```json title="xpengagent.json" {7-11}
{
  "$schema": "https://xpengagent.cc.cd/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
xpengagent mcp auth my-oauth-server
```
...
```bash
xpengagent mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil XPENGagent lagre tokens sikkert i `~/.local/share/xpengagent/mcp-auth.json`.
...
xpengagent mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i XPENGagent, sammen med innebygde verktøy. Så du kan administrere dem gjennom XPENGagent-konfigurasjonen som alle andre verktøy.
...
```json title="xpengagent.json" {11, 14-18}
{
  "$schema": "https://xpengagent.cc.cd/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="xpengagent.json" {4-8}
{
  "$schema": "https://xpengagent.cc.cd/config.json",
  "mcp": {
    "sentry": {
...
```bash
xpengagent mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble XPENGagent til Sentry-kontoen din.
...
```json title="xpengagent.json" {4-7}
{
  "$schema": "https://xpengagent.cc.cd/config.json",
  "mcp": {
    "context7": {
...
```json title="xpengagent.json" {7-9}
{
  "$schema": "https://xpengagent.cc.cd/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver XPENGagent-samtale oppført som en separat økt.
