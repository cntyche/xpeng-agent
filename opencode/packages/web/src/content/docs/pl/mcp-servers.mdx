...
Możesz dodać dodatkowe informacje do xpengagent za pomocą _Model Context Protocol_, w skrócie MCP. xpengagent obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [xpengagent Config](https://xpengagent.cc.cd/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/xpengagent`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
xpengagent automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, xpengagent:
...
Jeśli serwer wymaga uwierzytelnienia, xpengagent poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `xpengagent mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji xpengagent bezpieczne przechowa tokeny w `~/.local/share/xpengagent/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w xpengagent, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie xpengagent, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć xpengagent z kontem Sentry.
