---
title: Konfiguracija
description: Korištenje XPENGagent JSON konfiguracije.
---

Možete konfigurirati XPENGagent koristeći JSON konfiguracijski fajl.

---

## Format

XPENGagent podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/xpengagent`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/xpengagent/xpengagent.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`XPENGAGENT_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`xpengagent.json` u projektu) - postavke specifične za projekat
5. **`.xpengagent` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`XPENGAGENT_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.xpengagent` i `~/.config/xpengagent` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/xpengagent` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu XPENGagent konfiguraciju u `~/.config/xpengagent/xpengagent.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/xpengagent/tui.json`.
...
Dodajte `xpengagent.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se XPENGagent pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `XPENGAGENT_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `XPENGAGENT_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.xpengagent` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.xpengagent`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`xpengagent.cc.cd/config.json`**](https://xpengagent.cc.cd/config.json).

TUI konfiguracija koristi [**`xpengagent.cc.cd/tui.json`**](https://xpengagent.cc.cd/tui.json).
...
Koristite `XPENGAGENT_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `xpengagent.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `xpengagent serve` i `xpengagent web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš XPENGagent server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `xpengagent.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj XPENGagent konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, XPENGagent pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/xpengagent/agents/` ili `.xpengagent/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, XPENGagent će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`xpengagent run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/xpengagent/commands/` ili `.xpengagent/commands/`. [Saznajte više ovdje](/docs/commands).
...
XPENGagent će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, XPENGagent **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju XPENGagent sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.xpengagent/plugins/` ili `~/.config/xpengagent/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite XPENGagent da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
