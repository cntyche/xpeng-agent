...
- Tests cannot run from repo root (guard: `do-not-run-tests-from-root`); run from package dirs like `packages/xpengagent`.

## Type Checking

- Always run `bun typecheck` from package directories (e.g., `packages/xpengagent`), never `tsc` directly.
