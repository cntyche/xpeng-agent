...
import { createXpengagent, type ToolPart } from "@xpengagent/sdk"
...
console.log("🚀 Starting xpengagent server...")
const xpengagent = await createXpengagent({
  port: 0,
})
...
void (async () => {
  const events = await xpengagent.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new xpengagent session...")
    const { client, server } = xpengagent
    ...
    console.log("✅ Created xpengagent session:", createResult.data.id)
  ...
  console.log("📝 Sending to xpengagent:", message.text)
