...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. XPENGagent posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w XPENGagent.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `XPENGAGENT_EXPERIMENTAL_LSP_TOOL=true` (lub `XPENGAGENT_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy XPENGagent lub gdy zmienna środowiskowa `XPENGAGENT_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu XPENGagent:
