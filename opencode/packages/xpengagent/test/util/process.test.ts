...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.XPENGAGENT_TEST ?? "")'), {
      env: {
        XPENGAGENT_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "XPENGAGENT_TEST_SHELL"], {
      shell: true,
      env: {
        XPENGAGENT_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("XPENGAGENT_TEST_SHELL=ok")
  })
