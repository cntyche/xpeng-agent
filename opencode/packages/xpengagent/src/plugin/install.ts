...
export type PatchDeps = {
  ...
  files: (dir: string, name: "xpengagent" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".xpengagent")
}

function patchName(kind: Kind): "xpengagent" | "tui" {
  if (kind === "server") return "xpengagent"
  return "tui"
}
