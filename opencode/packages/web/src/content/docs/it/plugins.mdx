---
title: Plugin
description: Scrivi plugin per estendere XPENGagent.
---

I plugin ti permettono di estendere XPENGagent agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di XPENGagent.
...
- `.xpengagent/plugins/` - plugin a livello progetto
- `~/.config/xpengagent/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/xpengagent/node_modules/`.
...
1. Config globale (`~/.config/xpengagent/xpengagent.json`)
2. Config di progetto (`xpengagent.json`)
3. Directory plugin globale (`~/.config/xpengagent/plugins/`)
4. Directory plugin di progetto (`.xpengagent/plugins/`)
...
XPENGagent esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di xpengagent per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere xpengagent.
...
:::note
Se usi l'app desktop di XPENGagent, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a xpengagent di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a xpengagent:
...
L'helper `tool` crea uno strumento personalizzato che xpengagent può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in xpengagent insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://xpengagent.cc.cd/docs/sdk) per i dettagli.
